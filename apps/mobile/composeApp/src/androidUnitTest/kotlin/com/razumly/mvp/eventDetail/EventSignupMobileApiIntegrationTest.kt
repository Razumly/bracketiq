...
class EventSignupMobileApiIntegrationTest {
    ...
    fun givenSavedPreparation_whenSiteAndMobileResume_thenBothUseTheSameDraftAndMobileReadsRoom() = runTest(timeout = 10.minutes) {
        ...
        try {
            ...
            val publicEvent = otherAccount.eventRepository.getEvent(eventId).getOrThrow()
            assertEquals(8, publicEvent.teamSizeLimit)
            assertEquals(listOf("Indoor Volleyball"), publicEvent.sportIds)
            otherAccount.eventRepository.syncEventParticipants(publicEvent, null).getOrThrow()
            val cachedEvent = assertNotNull(otherAccount.database.getEventDao.getEventById(eventId))
            assertEquals(publicEvent.teamSizeLimit, cachedEvent.teamSizeLimit)
            assertEquals(publicEvent.sportIds, cachedEvent.sportIds)
            assertEquals(publicEvent.location, cachedEvent.location)
