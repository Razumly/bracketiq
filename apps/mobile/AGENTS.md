...
For every HTTP contract change, read the current `apps/site` schema before editing mobile DTOs. Update the DTO, JSON encoder, mapper, and API caller together. Add a focused wire-shape test and run a real mobile-to-site API integration test. A `MockEngine` test alone is not sufficient. Tests gated by `MVP_TEST_BACKEND_URL` must run in CI when the affected contract paths change.
...
When playoffs are enabled, default an absent `playoffTeamCount` to `3`. Current mobile editors must preserve explicit values and reject values below `3`.
