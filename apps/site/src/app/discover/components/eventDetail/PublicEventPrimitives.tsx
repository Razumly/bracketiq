...
export function ReadOnlyDetailsGrid({
  items,
}: {
  items: ReadOnlyDetailField[];
}) {
...
export function PublicEventSection({
  ...
  return (
    <section
      className={`border-b border-slate-200 py-7 first:pt-0 last:border-b-0 last:pb-0 ${className}`}
    >
      {title ? (
        <div className="mb-5">
          <h2 className="text-xl leading-tight font-bold text-slate-950">
            {title}
          </h2>
        </div>
      ) : null}
