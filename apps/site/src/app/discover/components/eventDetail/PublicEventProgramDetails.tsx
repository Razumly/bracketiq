            ...
            <PublicEventSection title="Choices" className="xl:h-full">
              {eligibleDivisionCount === 0 ? (
                <Alert color="yellow" variant="light">
                  No divisions are available for the selected registrant&apos;s
                  age.
                </Alert>
              ) : (
                <div className="divide-y divide-slate-200">
                  {divisionGroups.map((genderGroup) => {
                    const genderDivisionCount = genderGroup.ageGroups.reduce(
                      (count, ageGroup) =>
                        count +
                        ageGroup.skillGroups.reduce(
                          (skillCount, skillGroup) =>
                            skillCount + skillGroup.options.length,
                          0,
                        ),
                      0,
                    );
                    const genderHasSelected = genderGroup.ageGroups.some(
                      (ageGroup) =>
                        ageGroup.skillGroups.some((skillGroup) =>
                          skillGroup.options.some(isSelected),
                        ),
                    );
                    return (
                      <details
                      ...
                      >
                        ...
                        <div className="pb-2 pl-3">
                          {genderGroup.ageGroups.map((ageGroup) => {
                            const ageDivisionCount =
                              ageGroup.skillGroups.reduce(
                                (count, skillGroup) =>
                                  count + skillGroup.options.length,
                                0,
                              );
                            const ageHasSelected = ageGroup.skillGroups.some(
                              (skillGroup) =>
                                skillGroup.options.some(isSelected),
                            );
                            return (
                              <details
                                ...
                                open={
                                  ageHasSelected ||
                                  genderGroup.ageGroups.length === 1
                                }
                              >
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        <div className="grid grid-cols-1 gap-2 pb-2 pl-3">
                                          {skillGroup.options.map(
                                            (division) => {
                                              const selected =
                                                isSelected(division);
                                              const displaySkillLabel =
                                                skillGroup.options.length > 1
                                                  ? division.name
                                                  : skillGroup.label;
                                              return (
                                                <button
                    ...
                    schedulePreviewItems.map((item) => (
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          <Text fw={800} className="truncate text-slate-950">
                            {item.title}
                          </Text>
            ...
            <PublicEventSection title="Staff" className="h-full">
              <div className="grid grid-cols-1 gap-3">
                ...
                <PublicEventMetaPill
                  ...
                  value={formatReadOnlyValueList(
                    assistantHostNames,
                    "No assistant hosts assigned",
                  )}
                />
                <PublicEventMetaPill
                  label="Officials"
                ...
                <PublicEventMetaPill
                  ...
                  value={officialPositionsSummary}
                />
              </div>
            </PublicEventSection>
          ) : null}
