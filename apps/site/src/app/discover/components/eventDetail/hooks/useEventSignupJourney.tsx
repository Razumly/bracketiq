...
export function useEventSignupJourney({ event, user, progress, selectedTeamId, onTeamSelected, onContinueToReview }: {
    ...
    const [dialogProgressKey, setDialogProgressKey] = useState(progress.progressKey);
    ...
    if (dialogProgressKey !== progress.progressKey) {
        setDialogProgressKey(progress.progressKey);
        setMode('idle');
        setInviting(false);
    }
    useEffect(() => {
