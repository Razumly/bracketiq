...
const canManageRoster = async (profileId: string, session: { userId: string; isAdmin?: boolean }): Promise<boolean> => {
  if (session.isAdmin) return true;
  const invite = await prisma.invites.findFirst({
    where: { type: 'TEAM', userId: profileId, role: 'player', status: { in: ['PENDING', 'SENT', 'FAILED'] } },
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!(await canManageRoster(profileId, session))) return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
  try {
