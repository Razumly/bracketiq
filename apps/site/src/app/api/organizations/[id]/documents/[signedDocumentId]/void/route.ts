...
export async function POST(
  ...
  if (String(evidence.status ?? '').toUpperCase() === 'VOID') {
    return NextResponse.json({ evidenceId: evidence.id, status: 'VOID' }, { status: 200 });
  }

  const parsedBody = voidSchema.safeParse((await req.json().catch(() => null)) ?? {});
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Unable to void document evidence.';
