/** @jest-environment node */

import { NextRequest } from 'next/server';

const prismaMock = {
  $transaction: jest.fn(),
  events: {
    findUnique: jest.fn(),
    findMany: jest.fn(),
    update: jest.fn(),
  },
  divisions: {
    findMany: jest.fn(),
    update: jest.fn(),
  },
  fields: {
    findMany: jest.fn(),
  },
  teams: {
    findMany: jest.fn(),
    update: jest.fn(),
  },
  timeSlots: {
    findMany: jest.fn(),
  },
  userData: {
    findMany: jest.fn(),
  },
  matches: {
    findMany: jest.fn(),
    deleteMany: jest.fn(),
    upsert: jest.fn(),
  },
  leagueScoringConfigs: {
    findUnique: jest.fn(),
  },
};

const requireSessionMock = jest.fn();
const acquireEventLockMock = jest.fn();

jest.mock('@/lib/prisma', () => ({ prisma: prismaMock }));
jest.mock('@/lib/permissions', () => ({ requireSession: requireSessionMock }));
jest.mock('@/server/repositories/locks', () => ({
  acquireEventLock: (...args: any[]) => acquireEventLockMock(...args),
}));
jest.mock('@/server/matchScheduleNotifications', () => ({
  collectMatchScheduleChanges: jest.fn(() => []),
  notifyTeamsOfMatchScheduleUpdate: jest.fn(async () => undefined),
  snapshotMatchScheduleState: jest.fn(async () => new Map()),
}));

import { POST as schedulePost } from '@/app/api/events/[eventId]/schedule/route';

const jsonRequest = (url: string, body: any) =>
  new NextRequest(url, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
  });

describe('event schedule route - split divisions regression', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    prismaMock.$transaction.mockImplementation(async (fn: any) => fn(prismaMock));
    requireSessionMock.mockResolvedValue({ userId: 'host_1', isAdmin: false });
    acquireEventLockMock.mockResolvedValue(undefined);
    prismaMock.events.findMany.mockResolvedValue([]);
    prismaMock.events.update.mockResolvedValue(undefined);
    prismaMock.matches.findMany.mockResolvedValue([]);
    prismaMock.matches.deleteMany.mockResolvedValue(undefined);
    prismaMock.matches.upsert.mockResolvedValue(undefined);
    prismaMock.divisions.update.mockResolvedValue(undefined);
    prismaMock.teams.update.mockResolvedValue(undefined);
    prismaMock.userData.findMany.mockResolvedValue([]);
    prismaMock.leagueScoringConfigs.findUnique.mockResolvedValue(null);
  });

  it('keeps matches separated by division when scheduling a split-division league', async () => {
    prismaMock.events.findUnique.mockResolvedValue({
      id: 'event_1',
      createdAt: new Date('2026-01-01T00:00:00.000Z'),
      updatedAt: new Date('2026-01-01T00:00:00.000Z'),
      name: 'Split Division League',
      description: 'Regression coverage for split divisions',
      start: new Date('2026-01-03T09:00:00.000Z'),
      end: new Date('2026-02-28T13:00:00.000Z'),
      location: 'Main Gym',
      coordinates: null,
      price: 0,
      minAge: null,
      maxAge: null,
      rating: null,
      imageId: '',
      hostId: 'host_1',
      state: 'UNPUBLISHED',
      maxParticipants: 4,
      teamSizeLimit: 2,
      restTimeMinutes: 0,
      teamSignup: true,
      singleDivision: false,
      waitListIds: [],
      freeAgentIds: [],
      teamIds: ['team_beginner_1', 'team_beginner_2', 'team_advanced_1', 'team_advanced_2'],
      userIds: [],
      fieldIds: ['field_beginner', 'field_advanced'],
      timeSlotIds: ['slot_beginner', 'slot_advanced'],
      officialIds: [],
      cancellationRefundHours: null,
      registrationCutoffHours: null,
      seedColor: null,
      eventType: 'LEAGUE',
      sportIds: ['sport_1'],
      leagueScoringConfigId: null,
      organizationId: null,
      requiredTemplateIds: [],
      allowPaymentPlans: false,
      installmentCount: 0,
      installmentDueDates: [],
      installmentAmounts: [],
      allowTeamSplitDefault: false,
      divisions: ['beginner', 'advanced'],
      gamesPerOpponent: 1,
      includePlayoffs: false,
      playoffTeamCount: 0,
      doTeamsOfficiate: false,
      usesSets: false,
      matchDurationMinutes: 60,
      setDurationMinutes: 0,
      setsPerMatch: 0,
      pointsToVictory: [],
      winnerSetCount: null,
      loserSetCount: null,
      doubleElimination: false,
      fieldCount: 2,
      prize: null,
      winnerBracketPointsToVictory: [],
      loserBracketPointsToVictory: [],
    });

    prismaMock.divisions.findMany.mockResolvedValue([
      {
        id: 'event_1__division__beginner',
        key: 'beginner',
        name: 'Beginner',
        fieldIds: ['field_beginner'],
        kind: 'LEAGUE',
        teamIds: ['team_beginner_1', 'team_beginner_2'],
      },
      {
        id: 'event_1__division__advanced',
        key: 'advanced',
        name: 'Advanced',
        fieldIds: ['field_advanced'],
        kind: 'LEAGUE',
        teamIds: ['team_advanced_1', 'team_advanced_2'],
      },
    ]);

    prismaMock.fields.findMany.mockResolvedValue([
      {
        id: 'field_beginner',
        organizationId: null,
        divisions: ['beginner'],
        name: 'Court Beginner',
      },
      {
        id: 'field_advanced',
        organizationId: null,
        divisions: ['advanced'],
        name: 'Court Advanced',
      },
    ]);

    prismaMock.teams.findMany.mockResolvedValue([
      {
        id: 'team_beginner_1',
        seed: 1,
        captainId: 'captain_beginner_1',
        division: 'beginner',
        wins: 0,
        losses: 0,
        name: 'Beginner Team 1',
        playerIds: [],
      },
      {
        id: 'team_beginner_2',
        seed: 2,
        captainId: 'captain_beginner_2',
        division: 'beginner',
        wins: 0,
        losses: 0,
        name: 'Beginner Team 2',
        playerIds: [],
      },
      {
        id: 'team_advanced_1',
        seed: 1,
        captainId: 'captain_advanced_1',
        division: 'advanced',
        wins: 0,
        losses: 0,
        name: 'Advanced Team 1',
        playerIds: [],
      },
      {
        id: 'team_advanced_2',
        seed: 2,
        captainId: 'captain_advanced_2',
        division: 'advanced',
        wins: 0,
        losses: 0,
        name: 'Advanced Team 2',
        playerIds: [],
      },
    ]);

    prismaMock.timeSlots.findMany.mockResolvedValue([
      {
        id: 'slot_beginner',
        dayOfWeek: 5,
        daysOfWeek: [5],
        startDate: new Date('2026-01-03T00:00:00.000Z'),
        endDate: new Date('2026-02-28T00:00:00.000Z'),
        repeating: true,
        startTimeMinutes: 9 * 60,
        endTimeMinutes: 13 * 60,
        scheduledFieldId: 'field_beginner',
        divisions: ['beginner'],
      },
      {
        id: 'slot_advanced',
        dayOfWeek: 5,
        daysOfWeek: [5],
        startDate: new Date('2026-01-03T00:00:00.000Z'),
        endDate: new Date('2026-02-28T00:00:00.000Z'),
        repeating: true,
        startTimeMinutes: 9 * 60,
        endTimeMinutes: 13 * 60,
        scheduledFieldId: 'field_advanced',
        divisions: ['advanced'],
      },
    ]);

    const res = await schedulePost(
      jsonRequest('http://localhost/api/events/event_1/schedule', {}),
      { params: Promise.resolve({ eventId: 'event_1' }) },
    );
    const json = await res.json();

    expect(res.status).toBe(200);
    expect(prismaMock.matches.upsert).toHaveBeenCalledTimes(2);

    const persistedDivisions = prismaMock.matches.upsert.mock.calls
      .map((call) => call[0].create.division)
      .sort();
    expect(persistedDivisions).toEqual(['event_1__division__advanced', 'event_1__division__beginner']);

    const returnedDivisions = (Array.isArray(json.matches) ? json.matches : [])
      .map((match: any) => match.division)
      .filter(Boolean)
      .sort();
    expect(returnedDivisions).toEqual(['event_1__division__advanced', 'event_1__division__beginner']);
  });

  it('blocks split-division scheduling when event teams are unassigned', async () => {
    prismaMock.events.findUnique.mockResolvedValue({
      id: 'event_1',
      createdAt: new Date('2026-01-01T00:00:00.000Z'),
      updatedAt: new Date('2026-01-01T00:00:00.000Z'),
      name: 'Split Division League',
      description: 'Regression coverage for split divisions',
      start: new Date('2026-01-03T09:00:00.000Z'),
      end: new Date('2026-02-28T13:00:00.000Z'),
      location: 'Main Gym',
      coordinates: null,
      price: 0,
      minAge: null,
      maxAge: null,
      rating: null,
      imageId: '',
      hostId: 'host_1',
      state: 'UNPUBLISHED',
      maxParticipants: 4,
      teamSizeLimit: 2,
      restTimeMinutes: 0,
      teamSignup: true,
      singleDivision: false,
      waitListIds: [],
      freeAgentIds: [],
      teamIds: ['team_beginner_1', 'team_beginner_2', 'team_advanced_1', 'team_advanced_2'],
      userIds: [],
      fieldIds: ['field_beginner', 'field_advanced'],
      timeSlotIds: ['slot_beginner', 'slot_advanced'],
      officialIds: [],
      cancellationRefundHours: null,
      registrationCutoffHours: null,
      seedColor: null,
      eventType: 'LEAGUE',
      sportIds: ['sport_1'],
      leagueScoringConfigId: null,
      organizationId: null,
      requiredTemplateIds: [],
      allowPaymentPlans: false,
      installmentCount: 0,
      installmentDueDates: [],
      installmentAmounts: [],
      allowTeamSplitDefault: false,
      divisions: ['beginner', 'advanced'],
      gamesPerOpponent: 1,
      includePlayoffs: false,
      playoffTeamCount: 0,
      doTeamsOfficiate: false,
      usesSets: false,
      matchDurationMinutes: 60,
      setDurationMinutes: 0,
      setsPerMatch: 0,
      pointsToVictory: [],
      winnerSetCount: null,
      loserSetCount: null,
      doubleElimination: false,
      fieldCount: 2,
      prize: null,
      winnerBracketPointsToVictory: [],
      loserBracketPointsToVictory: [],
    });

    prismaMock.divisions.findMany.mockResolvedValue([
      {
        id: 'event_1__division__beginner',
        key: 'beginner',
        name: 'Beginner',
        fieldIds: ['field_beginner'],
        kind: 'LEAGUE',
        teamIds: ['team_beginner_1'],
      },
      {
        id: 'event_1__division__advanced',
        key: 'advanced',
        name: 'Advanced',
        fieldIds: ['field_advanced'],
        kind: 'LEAGUE',
        teamIds: ['team_advanced_1', 'team_advanced_2'],
      },
    ]);

    prismaMock.fields.findMany.mockResolvedValue([
      {
        id: 'field_beginner',
        organizationId: null,
        divisions: ['beginner'],
        name: 'Court Beginner',
      },
      {
        id: 'field_advanced',
        organizationId: null,
        divisions: ['advanced'],
        name: 'Court Advanced',
      },
    ]);

    prismaMock.teams.findMany.mockResolvedValue([
      {
        id: 'team_beginner_1',
        seed: 1,
        captainId: 'captain_beginner_1',
        division: 'beginner',
        wins: 0,
        losses: 0,
        name: 'Beginner Team 1',
        playerIds: [],
      },
      {
        id: 'team_beginner_2',
        seed: 2,
        captainId: 'captain_beginner_2',
        division: 'beginner',
        wins: 0,
        losses: 0,
        name: 'Beginner Team 2',
        playerIds: [],
      },
      {
        id: 'team_advanced_1',
        seed: 1,
        captainId: 'captain_advanced_1',
        division: 'advanced',
        wins: 0,
        losses: 0,
        name: 'Advanced Team 1',
        playerIds: [],
      },
      {
        id: 'team_advanced_2',
        seed: 2,
        captainId: 'captain_advanced_2',
        division: 'advanced',
        wins: 0,
        losses: 0,
        name: 'Advanced Team 2',
        playerIds: [],
      },
    ]);

    prismaMock.timeSlots.findMany.mockResolvedValue([
      {
        id: 'slot_beginner',
        dayOfWeek: 5,
        daysOfWeek: [5],
        startDate: new Date('2026-01-03T00:00:00.000Z'),
        endDate: new Date('2026-02-28T00:00:00.000Z'),
        repeating: true,
        startTimeMinutes: 9 * 60,
        endTimeMinutes: 13 * 60,
        scheduledFieldId: 'field_beginner',
        divisions: ['beginner'],
      },
      {
        id: 'slot_advanced',
        dayOfWeek: 5,
        daysOfWeek: [5],
        startDate: new Date('2026-01-03T00:00:00.000Z'),
        endDate: new Date('2026-02-28T00:00:00.000Z'),
        repeating: true,
        startTimeMinutes: 9 * 60,
        endTimeMinutes: 13 * 60,
        scheduledFieldId: 'field_advanced',
        divisions: ['advanced'],
      },
    ]);

    const res = await schedulePost(
      jsonRequest('http://localhost/api/events/event_1/schedule', {}),
      { params: Promise.resolve({ eventId: 'event_1' }) },
    );
    const json = await res.json();

    expect(res.status).toBe(400);
    const errorMessage = String(json.error ?? '');
    expect(errorMessage).toContain('Unassigned teams');
    expect(errorMessage).toContain('Beginner Team 2');
    expect(errorMessage).not.toContain('team_beginner_2');
    expect(prismaMock.matches.upsert).not.toHaveBeenCalled();
  });

  it('returns a clear error when split playoffs are enabled without playoff divisions', async () => {
    prismaMock.events.findUnique.mockResolvedValue({
      id: 'event_1',
      createdAt: new Date('2026-01-01T00:00:00.000Z'),
      updatedAt: new Date('2026-01-01T00:00:00.000Z'),
      name: 'Split Playoffs Missing Divisions',
      description: 'Regression coverage for split playoffs config',
      start: new Date('2026-01-03T09:00:00.000Z'),
      end: new Date('2026-02-28T13:00:00.000Z'),
      location: 'Main Gym',
      coordinates: null,
      price: 0,
      minAge: null,
      maxAge: null,
      rating: null,
      imageId: '',
      hostId: 'host_1',
      state: 'UNPUBLISHED',
      maxParticipants: 4,
      teamSizeLimit: 2,
      restTimeMinutes: 0,
      teamSignup: true,
      singleDivision: false,
      waitListIds: [],
      freeAgentIds: [],
      teamIds: ['team_beginner_1', 'team_beginner_2', 'team_beginner_3', 'team_beginner_4'],
      userIds: [],
      fieldIds: ['field_beginner'],
      timeSlotIds: ['slot_beginner'],
      officialIds: [],
      cancellationRefundHours: null,
      registrationCutoffHours: null,
      seedColor: null,
      eventType: 'LEAGUE',
      sportIds: ['sport_1'],
      leagueScoringConfigId: null,
      organizationId: null,
      requiredTemplateIds: [],
      allowPaymentPlans: false,
      installmentCount: 0,
      installmentDueDates: [],
      installmentAmounts: [],
      allowTeamSplitDefault: false,
      divisions: ['beginner'],
      gamesPerOpponent: 1,
      includePlayoffs: true,
      splitLeaguePlayoffDivisions: true,
      playoffTeamCount: 3,
      doTeamsOfficiate: false,
      usesSets: false,
      matchDurationMinutes: 60,
      setDurationMinutes: 0,
      setsPerMatch: 0,
      pointsToVictory: [],
      winnerSetCount: null,
      loserSetCount: null,
      doubleElimination: false,
      fieldCount: 1,
      prize: null,
      winnerBracketPointsToVictory: [],
      loserBracketPointsToVictory: [],
    });

    prismaMock.divisions.findMany.mockResolvedValue([
      {
        id: 'event_1__division__beginner',
        key: 'beginner',
        name: 'Beginner',
        fieldIds: ['field_beginner'],
        kind: 'LEAGUE',
        teamIds: ['team_beginner_1', 'team_beginner_2', 'team_beginner_3', 'team_beginner_4'],
      },
    ]);

    prismaMock.fields.findMany.mockResolvedValue([
      {
        id: 'field_beginner',
        organizationId: null,
        divisions: ['beginner'],
        name: 'Court Beginner',
      },
    ]);

    prismaMock.teams.findMany.mockResolvedValue([
      {
        id: 'team_beginner_1',
        seed: 1,
        captainId: 'captain_beginner_1',
        division: 'beginner',
        wins: 0,
        losses: 0,
        name: 'Beginner Team 1',
        playerIds: [],
      },
      {
        id: 'team_beginner_2',
        seed: 2,
        captainId: 'captain_beginner_2',
        division: 'beginner',
        wins: 0,
        losses: 0,
        name: 'Beginner Team 2',
        playerIds: [],
      },
      {
        id: 'team_beginner_3',
        seed: 3,
        captainId: 'captain_beginner_3',
        division: 'beginner',
        wins: 0,
        losses: 0,
        name: 'Beginner Team 3',
        playerIds: [],
      },
      {
        id: 'team_beginner_4',
        seed: 4,
        captainId: 'captain_beginner_4',
        division: 'beginner',
        wins: 0,
        losses: 0,
        name: 'Beginner Team 4',
        playerIds: [],
      },
    ]);

    prismaMock.timeSlots.findMany.mockResolvedValue([
      {
        id: 'slot_beginner',
        dayOfWeek: 5,
        daysOfWeek: [5],
        startDate: new Date('2026-01-03T00:00:00.000Z'),
        endDate: new Date('2026-02-28T00:00:00.000Z'),
        repeating: true,
        startTimeMinutes: 9 * 60,
        endTimeMinutes: 13 * 60,
        scheduledFieldId: 'field_beginner',
        divisions: ['beginner'],
      },
    ]);

    const res = await schedulePost(
      jsonRequest('http://localhost/api/events/event_1/schedule', {}),
      { params: Promise.resolve({ eventId: 'event_1' }) },
    );
    const json = await res.json();

    expect(res.status).toBe(400);
    expect(String(json.error ?? '')).toContain('Split playoff divisions are enabled');
    expect(prismaMock.matches.upsert).not.toHaveBeenCalled();
  });
});
