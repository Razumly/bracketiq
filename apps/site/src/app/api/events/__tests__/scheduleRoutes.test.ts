...
describe('schedule routes', () => {
  ...
  beforeEach(() => {
    ...
    prismaMock.eventTeamStaffAssignments.findMany.mockResolvedValue([]);
  ...
  it('notifies affected teams with a generic message after an existing event schedule is updated', async () => {
    requireSessionMock.mockResolvedValue({ userId: 'host_1', isAdmin: false });
    prismaMock.events.findUnique.mockResolvedValue({
      id: 'event_1',
      hostId: 'host_1',
    ...
    const team1 = { id: 'team_1', name: 'Alpha', captainId: 'captain_1', playerIds: ['player_1'] };
    const team2 = { id: 'team_2', name: 'Bravo', captainId: 'captain_2', playerIds: ['player_2'] };
    loadEventWithRelationsMock.mockResolvedValue({
      id: 'event_1',
      name: 'Spring League',
      eventType: 'LEAGUE',
    ...
    rescheduleEventMatchesPreservingLocksMock.mockReturnValue({
      event: { id: 'event_1', name: 'Spring League' },
      matches: [{
        id: 'match_1',
        matchId: 1,
        start: new Date('2026-05-01T12:00:00.000Z'),
        end: new Date('2026-05-01T13:00:00.000Z'),
        field: { id: 'field_1' },
        team1,
        team2,
      }],
    ...
    expect(sendPushToUsersMock).toHaveBeenCalledWith(expect.objectContaining({
      userIds: ['captain_1', 'manager_1', 'player_1', 'captain_2', 'manager_2', 'player_2'],
      title: 'Spring League schedule updated',
      body: 'Spring League has updated the schedule. Please review the changes.',
  ...
  it('accepts player incidents when older roster registration rows are not linked to the event team', async () => {
    requireSessionMock.mockResolvedValue({ userId: 'official_1', isAdmin: false });
    prismaMock.events.findUnique.mockResolvedValue({
      id: 'event_1',
      hostId: 'host_1',
  ...
  it('adds a player-recorded scoring incident through the dedicated incident endpoint', async () => {
    requireSessionMock.mockResolvedValue({ userId: 'official_1', isAdmin: false });
    prismaMock.events.findUnique.mockResolvedValue({
      id: 'event_1',
      hostId: 'host_1',
  ...
  it('rejects scoring incidents when the rules use direct score writes', async () => {
    requireSessionMock.mockResolvedValue({ userId: 'official_1', isAdmin: false });
    prismaMock.events.findUnique.mockResolvedValue({
      id: 'event_1',
      hostId: 'host_1',
  ...
  it('persists a retried scoring incident during segment confirmation without double-counting', async () => {
    requireSessionMock.mockResolvedValue({ userId: 'official_1', isAdmin: false });
    prismaMock.events.findUnique.mockResolvedValue({
      id: 'event_1',
      hostId: 'host_1',
  ...
  it('deduplicates a scoring incident create that is resent after it already persisted', async () => {
    requireSessionMock.mockResolvedValue({ userId: 'official_1', isAdmin: false });
    prismaMock.events.findUnique.mockResolvedValue({
      id: 'event_1',
      hostId: 'host_1',
    ...
    expect(res.status).toBe(200);
    const savedMatch = saveMatchesMock.mock.calls[0][1][0];
    expect(savedMatch.segments[0].scores).toEqual({ team_1: 1, team_2: 0 });
    expect(savedMatch.team1Points).toEqual([1]);
    expect(savedMatch.incidents).toHaveLength(1);
    expect(savedMatch.incidents[0]).toEqual(expect.objectContaining({
      id: 'incident_retry_1',
      linkedPointDelta: 1,
    }));
  });
  ...
  it('bulk updates matches atomically when user is host', async () => {
        ...
        matches: [
          {
            ...
            locked: true,
            status: 'IN_PROGRESS',
            actualStart: '2026-05-01T10:00:00.000Z',
            winnerEventTeamId: null,
            segments: [{
              id: 'match_1_segment_1',
              sequence: 1,
    ...
    expect(saveMatchesMock).toHaveBeenCalledTimes(1);
    const savedMatch = saveMatchesMock.mock.calls[0][1].find((entry: any) => entry.id === 'match_1');
    expect(savedMatch.status).toBe('IN_PROGRESS');
    expect(savedMatch.actualStart).toEqual(new Date('2026-05-01T10:00:00.000Z'));
    expect(savedMatch.segments).toEqual([
      expect.objectContaining({
        id: 'match_1_segment_1',
        sequence: 1,
  ...
  it('notifies match teams with the match label when a single bulk match update reschedules a match', async () => {
    ...
    expect(sendPushToUsersMock).toHaveBeenCalledWith(expect.objectContaining({
      userIds: ['captain_1', 'manager_1', 'player_1', 'captain_2', 'manager_2', 'player_2'],
      title: 'Spring Tournament match updated',
      body: 'Spring Tournament has rescheduled Match 7: Alpha vs Bravo. Please review the changes.',
      data: expect.objectContaining({
        type: 'match_schedule_update',
        eventId: 'event_1',
        updateScope: 'match',
  ...
  it('notifies teams when finalizing a bracket match adds a team to the next match', async () => {
    requireSessionMock.mockResolvedValue({ userId: 'host_1', isAdmin: false });
    prismaMock.events.findUnique.mockResolvedValue({
      id: 'event_1',
      hostId: 'host_1',
    ...
    const team1 = { id: 'team_1', name: 'Alpha', captainId: 'captain_1', playerIds: ['player_1'] };
    const team2 = { id: 'team_2', name: 'Bravo', captainId: 'captain_2', playerIds: ['player_2'] };
    const team3 = { id: 'team_3', name: 'Charlie', captainId: 'captain_3', playerIds: ['player_3'] };
    const nextMatch: any = {
      id: 'match_2',
      matchId: 2,
      team1: null,
    ...
    expect(res.status).toBe(200);
    expect(sendPushToUsersMock).toHaveBeenCalledWith(expect.objectContaining({
      userIds: ['captain_1', 'manager_1', 'player_1', 'captain_3', 'manager_3', 'player_3'],
      title: 'Bracket Finals match updated',
      body: 'Bracket Finals has updated Match 2: Alpha vs Charlie. Please review the changes.',
      data: expect.objectContaining({
        type: 'match_schedule_update',
        eventId: 'event_1',
        updateScope: 'match',
