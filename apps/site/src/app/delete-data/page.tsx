      ...
      <ul className="landing-section-copy mt-4 space-y-3 text-sm sm:text-base">
        {items.map((item) => (
          <li key={item} className="flex gap-3">
            <span
              aria-hidden="true"
              className="mt-1 text-[0.8rem] text-[var(--landing-accent-text)]"
            >
              ●
            </span>
            <span>{item}</span>
    ...
    <main className="landing-root min-h-screen">
      ...
      <header className="landing-header sticky top-0 z-20 backdrop-blur-lg">
        <div className="container-responsive flex items-center justify-between py-4">
          <Link
            href="/"
            className="landing-brand text-lg font-semibold tracking-wide"
          >
            BracketIQ
          </Link>
          <Link
        ...
        <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <h1 className="landing-title max-w-3xl text-4xl leading-tight font-semibold sm:text-5xl">
                Delete your BracketIQ account data.
              </h1>
              <p className="landing-copy max-w-2xl text-base sm:text-lg">
                This page explains how users of{" "}
                <strong>BracketIQ by Razumly</strong> can request account
                deletion, what data is deleted, what data is retained, and the
          ...
          <aside className="landing-cta rounded-3xl p-6 sm:p-7">
            ...
            <h2 className="landing-section-title mt-3 text-2xl font-semibold">
              Refund requests matter.
            </h2>
            <div className="landing-cta-copy mt-4 space-y-3 text-sm sm:text-base">
              <p>
                If there is a pending refund request owed to you, it must be
                addressed before BracketIQ can complete account deletion.
              </p>
              <p>
                If there is a pending refund request you still owe as a host or
                organizer, that must also be resolved before the deletion can
                finish.
              </p>
            </div>
          </aside>
      ...
      <section className="container-responsive relative grid gap-6 pb-20 md:grid-cols-2">
        <SectionCard
          title="Data deleted when the request is processed"
          items={deletedData}
        />
        <SectionCard
          title="Data kept briefly before deletion"
          items={delayedDeletion}
        />
        <SectionCard
          title="Data kept for historical, billing, and compliance records"
