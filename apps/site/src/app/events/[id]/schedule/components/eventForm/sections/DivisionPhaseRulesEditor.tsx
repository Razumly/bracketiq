...
type DivisionPhaseConfigurationControlsProps = {
  ...
  officialPositions?: Parameters<
    typeof MatchRulesSection
  >[0]["officialPositions"];
...
export const DivisionPhaseConfigurationControls = ({
  ...
  const segmentLabel =
    configuredRules.segmentLabel?.trim() ||
  ...
  const segmentBreakMinutes =
    settings.segmentBreakMinutes === undefined
      ? 0
      : settings.segmentBreakMinutes;
  const inheritedSegmentLength =
    inheritedRules.timekeeping?.segmentDurationMinutes ??
    (configuredMatchDurationMinutes != null && segmentCount > 0
      ? Math.max(
          1,
          Math.round(
            (configuredMatchDurationMinutes -
              (segmentBreakMinutes ?? 0) * Math.max(0, segmentCount - 1)) /
              segmentCount,
          ),
        )
      : null);
  const segmentLengthMinutes =
    settings.segmentLengthMinutes === undefined
      ? inheritedSegmentLength
      : settings.segmentLengthMinutes;
  const calculatedDurationMinutes = usesSets
    ? null
  ...
  const totalCycleMinutes =
    calculatedDurationMinutes == null
      ? null
      : calculatedDurationMinutes + Math.max(0, restTimeMinutes ?? 0);
  ...
  const updateSettings = (next: DivisionPhaseSettings) => {
    ...
    if (!usesSets) {
      onCalculatedDurationChange?.(
        calculateTimedMatchDurationMinutes({
          segmentCount:
            next.matchRulesOverride?.segmentCount ??
            inheritedRules.segmentCount ??
            1,
          segmentLengthMinutes:
            next.segmentLengthMinutes === undefined
              ? inheritedSegmentLength
              : next.segmentLengthMinutes,
          segmentBreakMinutes:
            next.segmentBreakMinutes === undefined
              ? 0
          ...
          <>
            ...
            <Text size="sm" c="dimmed" pb={8}>
              {calculatedDurationMinutes == null
                ...
                : `Calculated match duration: ${calculatedDurationMinutes} minutes. Total time with rest: ${totalCycleMinutes} minutes.`}
            </Text>
          </>
        ) : null}
        <Button
      ...
      >
        <Stack gap="lg">
          <MatchRulesSection
            ...
            onAutoCreatePointMatchIncidentsChange={(nextValue) =>
              updateSettings({
                ...settings,
