...
export default function TournamentBracketView({
  ...
  const matchesById = useMemo<Record<string, Match>>(
    () =>
      Object.fromEntries(
        Object.values(bracket.matches).map((match) => [match.$id, match]),
      ),
    [bracket.matches],
  );
  const conflictMatchIdSet = useMemo(
    () =>
      new Set(
        Object.keys(conflictMatchIdsById).filter(
          (matchId) => conflictMatchIdsById[matchId]?.length,
        ),
      ),
    [conflictMatchIdsById],
  );
  const resolveLinkFromAll = useCallback(
    (idValue: unknown, relationValue: unknown): Match | undefined => {
  ...
  const leaguePlayoffPlaceholderAssignments = useMemo<
  ...
  const teamHasCurrentUser = useCallback(
    (team: Match["team1"], fallbackTeamId?: string | null): boolean => {
      ...
      const players = Array.isArray((team as any)?.players)
        ? (team as any).players
        : [];
      if (
        players.some((player: unknown) => {
      ...
      const playerIds = Array.isArray((team as any)?.playerIds)
        ? (team as any).playerIds
        : [];
      if (
        playerIds.some((playerId: unknown) => {
          const normalizedPlayerId =
            typeof playerId === "string" ? playerId.trim() : "";
          return (
            normalizedPlayerId.length > 0 &&
            trackedUserIds.has(normalizedPlayerId)
          );
        })
      ) {
        return true;
      ...
      const captainId =
        extractEntityId((team as any)?.captain) ||
        (typeof (team as any)?.captainId === "string"
          ? (team as any).captainId.trim()
          : "");
  ...
  const getCurrentUserMatchHighlight = useCallback(
    (match: Match): CurrentUserMatchHighlight | undefined => {
      ...
      const matchOfficialId =
        extractEntityId(match.official) ||
        (typeof match.officialId === "string" ? match.officialId.trim() : "");
      const hasAssignedOfficialSlot =
        Array.isArray(match.officialIds) &&
      ...
      if (
        (matchOfficialId && trackedUserIds.has(matchOfficialId)) ||
        hasAssignedOfficialSlot
      ) {
        return "official";
      }
      ...
      if (
        teamHasCurrentUser(match.team1, match.team1Id) ||
  ...
  const matchInvolvesCurrentUser = useCallback(
    (match: Match): boolean => Boolean(getCurrentUserMatchHighlight(match)),
    [getCurrentUserMatchHighlight],
  );

  const matchHasHighlightedDivision = useCallback(
    (match: Match): boolean => {
      const divisionKey = toBracketDivisionKey(
        getBracketMatchDivisionId(match),
      );
  ...
  const renderResolvedMatchCard = useCallback(
    (match: Match, className: string) => {
      const team1Id =
        extractEntityId((match as any).team1) ||
        (typeof match.team1Id === "string" ? match.team1Id.trim() : "");
      const team2Id =
        extractEntityId((match as any).team2) ||
        (typeof match.team2Id === "string" ? match.team2Id.trim() : "");
      const teamOfficialId =
        extractEntityId((match as any).teamOfficial) ||
        (typeof match.teamOfficialId === "string"
          ? match.teamOfficialId.trim()
          : "");
      const officialId =
        extractEntityId((match as any).official) ||
        (typeof match.officialId === "string" ? match.officialId.trim() : "");
      const resolvedTeam1 =
        match.team1 && typeof match.team1 === "object"
          ? match.team1
          : team1Id
            ? (teamsById.get(team1Id) ?? null)
            : null;
      const resolvedTeam2 =
        match.team2 && typeof match.team2 === "object"
          ? match.team2
          : team2Id
            ? (teamsById.get(team2Id) ?? null)
            : null;
      const resolvedTeamOfficial =
        match.teamOfficial && typeof match.teamOfficial === "object"
          ? match.teamOfficial
          : teamOfficialId
            ? (teamsById.get(teamOfficialId) ?? null)
            : null;
      const resolvedOfficial =
        match.official && typeof match.official === "object"
          ? match.official
          : officialId
            ? (officialsById.get(officialId) ?? null)
            : null;
      const resolvedMatch: Match = {
        ...
        previousLeftMatch: resolveLinkFromAll(
          match.previousLeftId,
          match.previousLeftMatch,
        ),
        previousRightMatch: resolveLinkFromAll(
      ...
      const team1Placeholder =
        leaguePlayoffPlaceholderAssignments[`${resolvedMatch.$id}:team1`];
      const team2Placeholder =
        leaguePlayoffPlaceholderAssignments[`${resolvedMatch.$id}:team2`];

      return (
        <MatchCard
          match={resolvedMatch}
          onClick={
            clickable ? () => handleMatchClick(resolvedMatch) : undefined
          }
          canManage={false}
          className={className}
    ...
    >
      ...
      <Paper withBorder p="sm">
        <Group justify="space-between" align="center" wrap="nowrap" w="100%">
          <Group gap="xs">
            ...
            <Button variant="default" size="xs" onClick={handleZoomReset}>
              Reset
            </Button>
            {isPreview && (
            ...
            <Text size="xs" c="dimmed" className="hidden md:inline">
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden bg-white">
          ...
          >
            <BracketCanvas
              ...
              renderCard={(matchId) => {
                ...
                return match
                  ? renderResolvedMatchCard(match, "w-full h-full")
                  : null;
              }}
            />
          </div>
        ...
        {unplacedMatches.length > 0 && (
          <Paper
            withBorder
            className="h-full border-t-0 border-r-0 border-b-0 border-l"
            style={{
              width: isUnplacedDockCollapsed ? 44 : 332,
              minWidth: isUnplacedDockCollapsed ? 44 : 332,
            }}
          >
      ...
      {allowScoreUpdates &&
        ...
        currentSelectedMatch && (
          <ScoreUpdateModal
            ...
            team1Placeholder={
              leaguePlayoffPlaceholderAssignments[
                `${currentSelectedMatch.$id}:team1`
              ]
            }
            team2Placeholder={
              leaguePlayoffPlaceholderAssignments[
                `${currentSelectedMatch.$id}:team2`
              ]
            }
          />
        )}
