...
import { useForm, Resolver, type FieldPath } from "react-hook-form";
...
import { buildLeagueScheduleError } from "./eventForm/scheduleMessages";
import {
...
} from "./eventForm/editorContractAdapters";
import {
...
const SIMPLE_PLANNING_PAGE_IDS = new Set<EventSetupPageId>(["format"]);

const normalizedOfficialPositionSignature = (
  positions: Array<{ name: string; count: number }> | undefined,
): string[] =>
  (positions ?? [])
    .map(
      (position) =>
        `${position.name.trim().toLocaleLowerCase()}:${Number(position.count)}`,
    )
    .sort();
...
export const buildDefaultSetupChoices = (
  ...
  const configuredOfficialPositions = normalizedOfficialPositionSignature(
    values?.officialPositions,
  );
  const defaultOfficialPositions = normalizedOfficialPositionSignature(
    values?.sportConfig?.officialPositionTemplates,
  );
  const hasCustomOfficialPositions =
    configuredOfficialPositions.length > 0 &&
  ...
  return {
    ...
    useStaffAssignments: Boolean(
      values?.hostId ||
        values?.assistantHostIds?.length ||
        values?.pendingStaffInvites?.length,
    ),
  ...
  (
    ...
    const incomingEvent = useMemo(() => {
      ...
      formEvent.fields = (formEvent.fields ?? []).map((field) => {
        ...
        const catalogField = fieldId
          ? catalogFieldsById.get(fieldId)
          : undefined;
        return catalogField ? ({ ...catalogField, ...field } as Field) : field;
      });
      return formEvent;
    }, [
      snapshot.catalogs.fields,
      snapshot.draft,
      snapshot.eventId,
      snapshot.mode,
    ]);
    const snapshotOrganization = snapshot.catalogs.organizations.find(
      (entry) =>
        entry.id === snapshot.draft.basics.organizationId ||
        entry.$id === snapshot.draft.basics.organizationId,
    ...
    const teamSignupLocked =
      snapshot.immutable.fieldNames.includes("teamSignup");
    ...
    const { eventTagOptions, hydratedOrganization, setHydratedOrganization } =
    ...
    const emitDraftState = useCallback(
      (state: { draft: Partial<Event>; baselineDraft: Partial<Event> }) => {
        onDraftStateChange?.({
          draft: eventFormValuesToEditorDraft(
            state.draft as unknown as EventFormValues,
            registrationQuestionDrafts,
          ),
          baselineDraft: eventFormValuesToEditorDraft(
            state.baselineDraft as unknown as EventFormValues,
            registrationQuestionDrafts,
          ),
        });
      },
    ...
    const [setupMode, setSetupMode] =
      useState<EventSetupMode>(defaultSetupMode);
    const [currentSimplePageId, setCurrentSimplePageId] =
      useState<EventSetupPageId>("format");
    const [completedSimplePageIds, setCompletedSimplePageIds] = useState<
      Set<EventSetupPageId>
    >(() => new Set());
    const [simpleSetupChoices, setSimpleSetupChoices] =
      useState<EventSetupChoices>(() => buildDefaultSetupChoices(formValues));
    const [reportedFormValidationErrors, setReportedFormValidationErrors] =
      useState<FlattenedFormError[]>([]);
    const [externalValidationErrors, setExternalValidationErrors] = useState<
      FlattenedFormError[]
    >([]);
    const [pendingValidationRecovery, setPendingValidationRecovery] =
      useState<EventFormErrorLocation | null>(null);
    ...
    const sectionErrorCounts = useMemo(
      () =>
        Object.fromEntries(
          Object.entries(validationErrorIndex.byAdvancedSection).map(
    ...
    useEffect(() => {
      ...
      setSimpleSetupChoices((current) =>
        current.useRegistrationQuestions
          ? current
          : { ...current, useRegistrationQuestions: true },
      );
    }, [registrationQuestionDrafts.length]);

    const handleTryoutDivisionSelection = useCallback(
      (sourceDivisions: Division[]) => {
        ...
        const existingBySourceId = new Map(
          existingDetails
            ...
            .map(
              (division) =>
                [division.sourceDivisionId as string, division] as const,
            ),
        );
        ...
        setValue(
          "divisions",
          nextDetails.map((division) => division.id),
          { shouldDirty: true, shouldValidate: true },
        );
        setValue("singleDivision", false, {
          shouldDirty: true,
          shouldValidate: true,
        });
        setValue("teamSignup", false, {
          shouldDirty: true,
          shouldValidate: true,
        });
      },
    ...
    const handleTryoutPriceChange = useCallback(
      (sourceDivisionId: string, price: number) => {
        setValue(
          "divisionDetails",
    ...
    const { hasStripeAccount, manualPaymentsEnabled, pricingControlsEnabled } =
      paymentController;
    ...
    const organizationId =
      resolvedOrganization?.$id ?? eventData.organizationId;
    ...
    const selectedImageUrl = useMemo(
      () =>
        selectedImageId
          ? getEventImageUrl({ imageId: selectedImageId, width: 800 })
          : "",
      [selectedImageId],
    );
    ...
    const setupResolverInput = useMemo<EventSetupResolverInput>(
      () => ({
        ...
        includePoolPlay:
          ...
          Boolean(leagueData.includePlayoffs),
        ...
        workflow: isCreateMode ? "CREATE" : "EDIT",
    ...
    const currentSimplePage =
      simpleSetupPages.find((page) => page.id === currentSimplePageId) ??
      simpleSetupPages[0];
    const currentSimplePageIndex = simpleSetupPages.findIndex(
      (page) => page.id === currentSimplePageId,
    );
    ...
    const confirmSimpleSetupTransition = useCallback(
      (nextInput: EventSetupResolverInput): boolean => {
        const impact = describeEventSetupTransition(
          setupResolverInput,
          nextInput,
        );
        const affectsConfiguredPage = impact.pageIds.some((pageId) =>
          completedSimplePageIds.has(pageId),
        );
        if (
          !affectsConfiguredPage ||
    ...
    const invalidateSimpleSetupPages = useCallback(
      (pageIds: Iterable<EventSetupPageId>) => {
    ...
    const handleSimpleExternalRegistrationChange = useCallback(
      (checked: boolean, applyValue: (checked: boolean) => void) => {
        const nextInput = {
          ...setupResolverInput,
          isExternalRegistration: checked,
        };
        if (!confirmSimpleSetupTransition(nextInput)) return;
        invalidateSimpleSetupPages(
          describeEventSetupTransition(setupResolverInput, nextInput).pageIds,
    ...
    const handleSimplePlayoffPlanningChange = useCallback(
      (
        updates: Pick<
          EventSetupResolverInput,
          "includePlayoffs" | "includePoolPlay" | "splitLeaguePlayoffDivisions"
        >,
        applyValue: () => void,
      ) => {
    ...
    const updateSimpleSetupChoices = useCallback(
      (updates: Partial<EventSetupChoices>) => {
        ...
        const turningOffConfiguredData =
          (resolvedUpdates.paidRegistration === false &&
            (Number(eventData.price) > 0 ||
              (eventData.divisionDetails ?? []).some(
                (division) => Number(division.price) > 0,
              ))) ||
          (resolvedUpdates.useRequiredDocuments === false &&
            Boolean(eventData.requiredTemplateIds?.length)) ||
          (resolvedUpdates.useRegistrationQuestions === false &&
            registrationQuestionDrafts.length > 0) ||
          (resolvedUpdates.useStaffAssignments === false &&
            Boolean(
              eventData.assistantHostIds?.length ||
                eventData.pendingStaffInvites?.some((invite) =>
                  invite.roles.includes("ASSISTANT_HOST"),
                ),
            )) ||
          (resolvedUpdates.useDedicatedOfficials === false &&
            Boolean(
          ...
          (resolvedUpdates.useTeamCheckInAndRosterOperations === false &&
            Boolean(
        ...
        if (resolvedUpdates.paidRegistration === false) {
          ...
          setValue("registrationPaymentMode", "ONLINE", {
            shouldDirty: true,
            shouldValidate: true,
          });
          ...
          setValue("manualPaymentInstructions", "", {
            shouldDirty: true,
            shouldValidate: true,
          });
          setValue("cancellationRefundHours", null, {
            shouldDirty: true,
            shouldValidate: true,
          });
          setValue("allowPaymentPlans", false, {
            shouldDirty: true,
            shouldValidate: true,
          });
          ...
          setValue("installmentAmounts", [], {
            shouldDirty: true,
            shouldValidate: true,
          });
          setValue("installmentDueDates", [], {
            shouldDirty: true,
            shouldValidate: true,
          });
          setValue("installmentDueRelativeDays", [], {
            shouldDirty: true,
            shouldValidate: true,
          });
          setValue(
        ...
        if (resolvedUpdates.useRegistrationQuestions === false)
          setRegistrationQuestionDrafts([]);
        if (resolvedUpdates.useStaffAssignments === false) {
          setValue("assistantHostIds", [], {
            shouldDirty: true,
            shouldValidate: true,
          });
          setValue(
            ...
            (eventData.pendingStaffInvites ?? []).flatMap((invite) => {
              const roles = invite.roles.filter(
                (role) => role !== "ASSISTANT_HOST",
              );
        ...
        if (resolvedUpdates.useDedicatedOfficials === false) {
          setValue("officialIds", [], {
            shouldDirty: true,
            shouldValidate: true,
          });
          setValue("eventOfficials", [], {
            shouldDirty: true,
            shouldValidate: true,
          });
          setValue(
        ...
        if (
          ...
          setValue("teamCheckInMode", "EVENT", {
            shouldDirty: true,
            shouldValidate: true,
          });
        }
        if (resolvedUpdates.useTeamCheckInAndRosterOperations === false) {
          setValue("teamCheckInMode", "OFF", {
            shouldDirty: true,
            shouldValidate: true,
          });
          setValue("teamCheckInOpenMinutesBefore", 60, {
            shouldDirty: true,
            shouldValidate: true,
          });
          setValue("allowMatchRosterEdits", false, {
            shouldDirty: true,
            shouldValidate: true,
          });
    ...
    const selectSimpleSetupPage = useCallback(
      (pageId: EventSetupPageId) => {
        const page = simpleSetupPages.find(
          (candidate) => candidate.id === pageId,
        );
        if (!page) return;
        setCurrentSimplePageId(
          page.status === "locked" && page.prerequisitePageId
    ...
    const handleSimpleSetupBack = useCallback(() => {
      if (previousUsedSimplePage)
        setCurrentSimplePageId(previousUsedSimplePage.id);
    ...
    const handleSimpleSetupNext = useCallback(async () => {
      ...
      setCompletedSimplePageIds((current) =>
        new Set(current).add(currentSimplePageId),
      );
    ...
    const formSections = <EventFormSections {...formSectionsModel} />;
    ...
    const simplePageContent =
      currentSimplePageId === "review-publish" ? (
        <SimpleSetupReviewPage
          model={simpleReviewModel}
          onEditPage={selectSimpleSetupPage}
        />
        ...
        <SimpleSetupPlanningPage
          ...
          onIncludePlayoffsChange={(checked) =>
      ...
      <div className="space-y-3">
        ...
        >
          {setupMode === "SIMPLE" && validationErrorIndex.ordered.length > 0 ? (
            ...
            >
              {validationErrorIndex.ordered.length}{" "}
              {validationErrorIndex.ordered.length === 1
                ? "issue needs"
                : "issues need"}{" "}
              attention. {validationErrorIndex.ordered[0]?.message}
            </div>
          ) : null}
          {setupMode === "SIMPLE" ? (
            <SimpleSetupPageFrame
