...
const featureSections = [
  {
    ...
    storyCopy:
      "Build court and field blocks for events, leagues, and tournaments, then spot conflicts before the schedule goes live.",
    points: ["Courts + fields", "Conflict checks"],
    details: [
      "Place resources quickly",
      "See conflicts before publishing",
      "Keep the full day visible",
    ],
    webImage: {
  ...
  },
  {
    ...
    storyCopy:
      "Keep teams, waitlists, attendance, and participant status visible from registration through check-in.",
  ...
  },
  {
    ...
    storyCopy:
      "Collect registration fees online, match payment records, and keep refunds tied to the right team or player.",
    points: ["Checkout", "Refunds"],
    details: [
      "Collect from mobile",
      "Match Stripe records",
  ...
  },
  {
    ...
    storyCopy:
      "Attach waivers and agreements to registrations so staff can see who is cleared before play starts.",
  ...
  },
  {
    ...
    storyCopy:
      "Send announcements and group messages from the same event workspace that holds the schedule.",
    points: ["Chat", "Announcements"],
    details: [
      "Message the right group",
      "Send event updates",
  ...
  },
  {
    ...
    storyCopy:
      "Give players and staff a mobile-ready view of game times, locations, and schedule changes.",
...
const utilitySignals = [
  {
    label: "Web + mobile",
    detail: "One operating layer for staff and participants.",
    icon: MonitorSmartphone,
  },
  {
    label: "Free app access",
    detail: "Run events without a platform subscription.",
    icon: BadgeCheck,
  },
  {
    label: "1% payment fee",
    detail: "Fees only apply when payments are processed.",
    icon: CreditCard,
  },
  {
    label: "Website integration included",
    detail: "Publish live event data where your audience already goes.",
    icon: Globe2,
  },
];
...
const workflowSteps = [
  {
    ...
    detail:
      "Create the event, divisions, fields, documents, team rules, and registration paths from one clean admin surface.",
  },
  {
    label: "Publish",
    title: "Open registration and schedules",
    detail:
      "Let teams and players join, pay, sign documents, and see what is live on web, mobile, or your existing site.",
  },
  {
    label: "Run",
    title: "Manage game day from the same system",
    detail:
      "Update brackets, send announcements, keep chat moving, and make changes without rebuilding the operation.",
  },
];
...
function OperationsFeatureStory({
  landingImageProps,
}: {
  ...
  useMotionValueEvent(scrollYProgress, "change", (latest) => {
    ...
    const nextIndex = Math.min(
      STICKY_FEATURE_SECTIONS.length - 1,
      Math.floor(clamped * STICKY_FEATURE_SECTIONS.length + 0.08),
    );
  ...
  if (prefersReducedMotion) {
    return (
      <section
        id="operations"
        className="landing-anchor-section landing-operations-section"
      >
  ...
  return (
    <section
      id="operations"
      className="landing-anchor-section landing-operations-section relative overflow-clip"
    >
...
function FeatureStoryPage({
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute top-20 right-10 z-20 font-sans text-[7rem] leading-none font-light text-white/80 2xl:text-[8rem]">
        {String(index + 1).padStart(2, "0")}
      </div>

      <div className="relative z-10 mx-auto grid h-full max-w-[1840px] grid-cols-[0.44fr_1.56fr] items-center gap-16 px-12 pt-24 pb-32 2xl:gap-24 2xl:px-16">
        <aside className="min-w-0">
          <h2 className="max-w-2xl text-6xl leading-[0.94] font-normal text-white xl:text-[4.25rem] 2xl:text-7xl">
            {feature.title}
          </h2>

          <p className="mt-5 max-w-md text-base leading-7 font-semibold text-white/74 2xl:mt-7 2xl:text-lg 2xl:leading-8">
            {feature.storyCopy}
          </p>
          ...
          >
            {feature.points.map((point) => (
              <li
                key={point}
                className="flex min-h-[4rem] items-center gap-3 border-r border-white/18 px-5 last:border-r-0 2xl:min-h-[4.75rem] 2xl:px-6"
              >
                <span
          ...
          <ul className="mt-6 grid gap-3 2xl:mt-9 2xl:gap-4">
            {feature.details.map((detail) => (
              <li
                key={detail}
                className="flex items-center gap-3 text-base font-semibold text-white/86 2xl:text-lg"
              >
                <CheckCircle2
        ...
        {BRACKET_NODES.map((node, index) => {
          ...
          if (isFinal) {
            return (
              ...
              >
                ...
                <span
                  className={`text-[0.62rem] font-black ${isActive ? "text-white" : "text-white/52"}`}
                >
                  {node.title}
                </span>
              </div>
          ...
          return (
            ...
            >
              ...
              <span
                className={`text-[0.62rem] font-black ${isActive || isComplete ? "text-white" : "text-white/52"}`}
              >
                {node.title}
              </span>
            </div>
...
function StaticOperationsContent({
  landingImageProps,
}: {
  landingImageProps: LandingImageProps;
}) {
  ...
  useEffect(() => {
    ...
    const cards = Array.from(
      operationsElement.querySelectorAll<HTMLElement>(
  ...
  return (
    ...
    >
      <div className="max-w-3xl">
        ...
        <h2 className="landing-static-operations-title mt-4 text-5xl leading-[0.95] font-black text-slate-950 sm:text-6xl">
          From setup to final.
        </h2>

        <p className="landing-static-operations-copy mt-5 max-w-xl text-base leading-7 font-semibold text-slate-600">
          BracketIQ connects scheduling, teams, brackets, documents,
          communication, and live operations in one sports management workspace.
        </p>
      </div>
      ...
      <div className="landing-static-operations-grid mt-10 grid gap-5">
        {featureSections.map((feature, index) => {
          ...
          const featureCardStyle = {
            "--landing-feature-theme":
              FEATURE_STORY_THEMES[index] ?? FEATURE_STORY_THEMES[0],
          } as CSSProperties;
            ...
            >
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="landing-operation-static-title mt-5 text-3xl font-black text-slate-950">
                    {feature.title}
                  </p>
                </div>
              </div>
              ...
              <ul
                className="landing-operation-point-list"
                aria-label={`${feature.title} focus`}
              >
                {feature.points.map((point) => (
...
export default function LandingPage({
    ...
    try {
      ...
      setGuestError(
        error instanceof Error
          ? error.message
          : "Unable to start guest session right now.",
      );
    } finally {
  ...
  return (
    <div className="landing-root min-h-screen">
      <MarketingHeader
        brandHref={brandHref}
        anchorHrefPrefix={anchorHrefPrefix}
      />

      <main className="relative">
        <section className="landing-hero-section container-responsive pt-14 pb-16 lg:pt-20 lg:pb-24">
          ...
          <div
            className="landing-hero-copy mx-auto max-w-5xl space-y-7 text-center"
            data-reveal
          >
            ...
            <div className="space-y-5">
              <h1 className="landing-title mx-auto max-w-5xl text-5xl leading-none font-semibold sm:text-6xl lg:text-7xl">
                Run every event from one home.
              </h1>
              <p className="landing-copy mx-auto max-w-2xl text-base sm:text-lg">
                BracketIQ gives organizers one place to run events, leagues,
                tournaments, teams, payments, and participant updates.
              </p>
            </div>
            ...
            <div
              ...
              aria-label="Included platform capabilities"
            >
              {["Registration", "Scheduling", "Payments", "Updates"].map(
                (label) => (
          ...
          <div
            className="landing-command-wrap mx-auto mt-12 w-full max-w-7xl"
            data-reveal
            data-delay="1"
          >
            <div className="landing-command-console">
              ...
              <div
                className={`landing-hero-stack ${isHeroMediaHorizontal ? "landing-hero-stack-horizontal" : ""}`}
              >
        ...
        <section
          id="platform"
          className="landing-anchor-section landing-platform-section container-responsive py-20"
        >
          <div className="landing-platform-layout">
            ...
            {platformColumns.map((column) => {
              ...
              return (
                <article
                  key={column.label}
                  className={`landing-platform-card ${isMobileLayer ? "landing-platform-card-mobile" : "landing-platform-card-web"}`}
                >
                  <div className="landing-platform-card-copy">
                    ...
                    <a
                      href={isMobileLayer ? "#operations" : "#operations"}
                      className="landing-platform-link"
                    >
                      {isMobileLayer
                  ...
                  <div
                    className={
                      isMobileLayer
                        ? "landing-platform-device landing-platform-device-phone"
                        : "landing-platform-device landing-platform-device-laptop"
                    }
                  >
                    {isMobileLayer ? (
                      ...
                      <div className="landing-macbook-frame">
                        ...
                        <div
                          className="landing-macbook-base"
                          aria-hidden="true"
                        />
                      </div>
                    )}
        ...
        <section
          id="integrations"
          className="landing-anchor-section landing-post-scroll-section container-responsive pb-20"
        >
          <div className="landing-integration-panel">
            <div className="landing-integration-copy landing-section-heading-compact">
              ...
              <h2 className="landing-section-title mt-3">
                Your site stays live.
              </h2>
              <p className="landing-section-copy mt-5">
                Publish schedules, brackets, registration, payments, and
                documents from BracketIQ to the website your organization
                already uses.
              </p>
              <div
                className="landing-integration-chip-row"
                aria-label="Website integration capabilities"
              >
                {["Event pages", "Embeds", "Checkout", "Documents"].map(
                  (item) => (
            ...
            <div
              className="landing-integration-visual"
              aria-label="Connected integration services"
            >
              ...
              {integrations.map((integration) => (
                <article
                  ...
                  className={`landing-integration-logo ${integration.name === "Google Maps" ? "gap-2" : ""}`}
                >
                  <Image
                  ...
                  {integration.wordmarkText ? (
                    <span className="landing-pill-wordmark text-base font-semibold whitespace-nowrap">
                      {integration.wordmarkText}
                    </span>
                  ) : null}
                </article>
        ...
        >
          ...
          <div className="landing-use-case-row">
            <div className="landing-use-case-copy">
              <p className="landing-label">Use cases</p>
              <h3>
                One operating layer for sports programs, clubs, and event
                organizers.
              </h3>
              <p>
                Mix leagues, rentals, camps, clubs, and tournaments without
                changing systems for each format.
              </p>
              <div className="landing-use-case-list">
                {useCases.map((useCase) => (
                  <article key={useCase.label} className="landing-use-case">
                    <CheckCircle2 aria-hidden="true" className="h-4 w-4" />
                    <div>
                      <div className="landing-use-case-title">
                        {useCase.label}
                      </div>
                      <p className="landing-use-case-detail">
                        {useCase.detail}
                      </p>
                    </div>
                  </article>
        ...
        >
          <div className="landing-resource-panel">
            <div className="landing-resource-copy">
              ...
              <h2 className="landing-section-title mt-3">
                Playbooks for better event days.
              </h2>
              <p className="landing-section-copy mt-4">
                Short guides for scheduling, payments, and team updates.
              </p>
              <Link
                href="/guides/paid-pickup-event-payments"
                className="landing-btn-primary landing-btn-large mt-7"
              >
                Read payment guide
                <ArrowRight aria-hidden="true" className="h-4 w-4" />
              </Link>
            ...
            <div
              className="landing-resource-board"
            ...
            >
              <article className="landing-featured-guide">
                ...
                <div className="landing-featured-guide-title">
                  Paid pickup events
                </div>
                <ul className="landing-resource-point-list">
                  {["Set price", "Publish event", "Verify checkout"].map(
                    (item) => (
                      <li key={item}>
                        <CheckCircle2 aria-hidden="true" className="h-4 w-4" />
                        <span>{item}</span>
              ...
              <div className="landing-resource-highlight-grid">
                {resourceHighlights.map((item) => {
                  ...
                  return (
                    <article
                      key={item.label}
                      className="landing-resource-highlight"
                    >
                      ...
                      <div>
                        ...
                        <div className="landing-resource-highlight-title">
                          {item.title}
                        </div>
                        <p>{item.detail}</p>
                      </div>
        ...
        >
          <div className="landing-fees-panel">
            <div className="landing-fees-copy landing-section-heading-compact">
              ...
              <h2 className="landing-section-title mt-3">
                Free to use. Pay only on processing.
              </h2>
              <p className="landing-section-copy mt-4">
                Run events, leagues, and tournaments without a subscription.
                BracketIQ only takes a 1% fee when payments are processed.
              </p>
            </div>

            <div className="landing-payment-visual" aria-hidden="true">
              <span className="landing-money-float landing-money-float-one">
                $
              </span>
              <span className="landing-money-float landing-money-float-two">
                USD
              </span>
              <span className="landing-money-float landing-money-float-three">
                $
              </span>
              <div className="landing-payment-terminal">
                <div className="landing-payment-terminal-header">
                  <div>
                    <p>Payment flow</p>
                    <div className="landing-payment-terminal-title">
      ...
      <footer className="landing-final-cta relative overflow-hidden bg-slate-950 text-white">
        ...
        <div className="relative mx-auto max-w-7xl px-5 py-28 lg:px-8">
          <h2 className="max-w-5xl text-6xl leading-[0.92] font-black text-white sm:text-7xl lg:text-8xl">
            Run the whole sports calendar, not just the bracket.
          </h2>

          <p className="mt-6 max-w-2xl text-lg leading-8 font-semibold text-white/65">
            BracketIQ keeps schedules, teams, brackets, documents, updates, and
            live operations connected from setup through the final match.
          </p>
