...
type DateControlProps = FieldProps & { value?: Date | string | null; onChange?: (value: Date | null) => void; minDate?: Date; placeholder?: string; size?: string; clearable?: boolean; valueFormat?: string; highlightToday?: boolean; timePickerProps?: unknown; clearButtonProps?: React.ButtonHTMLAttributes<HTMLButtonElement>; popoverProps?: unknown; style?: React.CSSProperties };
...
function DateControl({ value, onChange, id, label, description, error, required, mb, mt, p, minDate, placeholder, clearable: _clearable, valueFormat: _valueFormat, highlightToday: _highlightToday, timePickerProps: _timePickerProps, clearButtonProps: _clearButtonProps, popoverProps: _popoverProps, style, size: _size, includeTime }: DateControlProps & { includeTime?: boolean }) { const resolvedId = useFieldId(id, label); return <FieldFrame id={resolvedId} label={label} description={description} error={error} required={required} mb={mb} mt={mt} p={p}><input id={resolvedId} type={includeTime ? 'datetime-local' : 'date'} value={dateInputValue(value, Boolean(includeTime))} min={minDate ? dateInputValue(minDate, Boolean(includeTime)) : undefined} placeholder={placeholder} required={required} aria-invalid={Boolean(error) || undefined} onChange={(event) => onChange?.(event.currentTarget.value ? new Date(event.currentTarget.value) : null)} className="h-11 min-h-11 w-full min-w-0 rounded-lg border border-input bg-background px-3 py-2 text-base text-foreground outline-none transition-colors focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring disabled:cursor-not-allowed disabled:bg-muted disabled:text-muted-foreground disabled:opacity-70 md:text-sm" style={style} /></FieldFrame>; }
