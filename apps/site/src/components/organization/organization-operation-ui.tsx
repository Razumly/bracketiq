  ...
  return (
    ...
    >
      <textarea
        ...
        aria-invalid={Boolean(error) || undefined}
        aria-describedby={props['aria-describedby'] ?? messageIds.describedBy}
        className={cn('min-h-24 w-full min-w-0 resize-y border border-input bg-background px-3 py-2.5 text-base text-foreground outline-none transition-colors placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring disabled:cursor-not-allowed disabled:bg-muted disabled:text-muted-foreground disabled:opacity-70 md:text-sm', radiusClass(radius, 'control'))}
        onChange={(event) => onChange?.(stableChangeEvent(event))}
      />
    </FieldFrame>
  );
    ...
    <FieldFrame id={resolvedId} label={label} description={description} error={error} errorProps={errorProps} required={required || withAsterisk} mb={mb} mt={mt} p={p} className={className} style={{ width: w, maxWidth: maw, ...style }}>
      <div ref={containerRef} className={cn('relative', controlRadius)}>
        ...
        <input
          ...
          className={cn('h-11 min-h-11 w-full min-w-0 rounded-lg border border-input bg-background px-3 py-2 pr-10 text-base text-foreground outline-none transition-colors placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring disabled:cursor-not-allowed disabled:bg-muted disabled:text-muted-foreground disabled:opacity-70 md:text-sm', leftSection && 'pl-9', rightSection && 'pr-9')}
        />
        ...
        {open && <div ref={listboxRef} id={listboxId} role="listbox" className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-lg border border-border bg-popover p-1 text-popover-foreground shadow-lg">{visibleOptions.length ? visibleOptions.map((option, index) => { const normalized = normalizeSelectOption(option); return <button type="button" role="option" id={`${listboxId}-option-${index}`} data-option-index={index} aria-selected={normalized.value === value} key={normalized.value} disabled={normalized.disabled} onMouseDown={(event) => event.preventDefault()} onMouseEnter={() => setActiveOptionIndex(index)} onClick={() => selectOption(option)} className={cn('block min-h-10 w-full rounded-md px-3 py-2 text-left text-sm hover:bg-muted disabled:cursor-not-allowed disabled:opacity-50', index === activeOptionIndex && 'bg-muted')}>{selectOptionLabel(option)}</button>; }) : <div className="px-3 py-2 text-sm text-muted-foreground">{_nothingFoundMessage ?? 'Nothing found'}</div>}</div>}
      </div>
    </FieldFrame>
  );
}
    ...
    <FieldFrame id={resolvedId} label={label} description={description} error={error} errorProps={errorProps} required={required || withAsterisk} mb={mb} mt={mt} p={p} className={className} style={{ width: w, maxWidth: maw, ...style }}>
      <div ref={containerRef} className={cn('relative', controlRadius)}>
        <input
          ...
          className={cn('h-11 min-h-11 w-full min-w-0 rounded-lg border border-input bg-background px-3 py-2 pr-10 text-base text-foreground outline-none transition-colors placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring disabled:cursor-not-allowed disabled:bg-muted disabled:text-muted-foreground disabled:opacity-70 md:text-sm')}
        />
  ...
  return (
    ...
    >
      <div ref={containerRef} className={cn('relative', radiusClass(radius, 'control'))}>
        <div className="flex min-h-11 flex-wrap items-center gap-1.5 rounded-lg border border-input bg-background px-2 py-1.5 focus-within:border-ring focus-within:ring-[3px] focus-within:ring-ring">
          {selected.map((tag) => (
