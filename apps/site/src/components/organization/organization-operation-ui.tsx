...
type ModalProps = { opened: boolean; onClose: () => void; title?: React.ReactNode; children?: React.ReactNode; centered?: boolean; size?: string; styles?: { content?: React.CSSProperties; body?: React.CSSProperties }; withCloseButton?: boolean; fullScreen?: boolean; closeOnClickOutside?: boolean; closeOnEscape?: boolean; embedded?: boolean };
export function Modal({ opened, onClose, title, children, size, styles, withCloseButton = true, fullScreen = false, closeOnClickOutside: _closeOnClickOutside, closeOnEscape: _closeOnEscape, embedded = false }: ModalProps) {
  if (embedded) {
    return opened ? <div data-slot="modal-embedded" className="flex h-full min-h-0 w-full min-w-0 flex-col" style={styles?.content}><div className="min-h-0 min-w-0 flex-1" style={styles?.body}>{children}</div></div> : null;
  }
