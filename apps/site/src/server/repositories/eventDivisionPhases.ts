...
export const syncEventPhaseParticipantsFromEntryDivisions = async (params: {
  client: PhasePersistenceClient;
  eventId: string;
}): Promise<void> => {
  const { client, eventId } = params;
  ...
  if (
    typeof divisions?.findMany !== 'function'
    || typeof sources?.findMany !== 'function'
    || typeof participants?.findMany !== 'function'
    || typeof participants.upsert !== 'function'
  ) {
    return;
  }

  const [entryRows, phaseRows, sourceRows, participantRows] = await Promise.all([
    divisions.findMany({
  ...
  const entryTeamIdsById = new Map(
    entryRows.map((row) => [row.id, asStringArray(row.teamIds)]),
  );
  const sourceEntryIdsByPhase = new Map<string, string[]>();
  for (const row of sourceRows) {
    const phaseId = String(row.phaseDivisionId ?? '').trim();
    const entryId = String(row.entryDivisionId ?? '').trim();
    if (!phaseId || !entryId) continue;
    const entryIds = sourceEntryIdsByPhase.get(phaseId) ?? [];
    if (!entryIds.includes(entryId)) entryIds.push(entryId);
    sourceEntryIdsByPhase.set(phaseId, entryIds);
  }
  const participantTeamIdsByPhase = new Map<string, string[]>();
  for (const row of participantRows) {
    const phaseId = String(row.phaseDivisionId ?? '').trim();
    const teamId = String(row.eventTeamId ?? '').trim();
    if (!phaseId || !teamId) continue;
    const teamIds = participantTeamIdsByPhase.get(phaseId) ?? [];
    if (!teamIds.includes(teamId)) teamIds.push(teamId);
    participantTeamIdsByPhase.set(phaseId, teamIds);
  }

  const phaseIds = phaseRows.map((row) => row.id);
  if (phaseIds.length && typeof participants.deleteMany === 'function') {
    await participants.deleteMany({
      where: { eventId, phaseDivisionId: { in: phaseIds } },
    });
  }
  for (const phase of phaseRows) {
    const phaseKind = String(phase.phase ?? '').toUpperCase();
    const sourceEntryIds = sourceEntryIdsByPhase.get(phase.id)
      ?? (phase.sourceDivisionId ? [phase.sourceDivisionId] : []);
    const participantTeamIds = phaseKind === 'POOL' && participantTeamIdsByPhase.has(phase.id)
      ? participantTeamIdsByPhase.get(phase.id) ?? []
      : phaseKind === 'POOL'
        ? asStringArray(phase.teamIds)
        : Array.from(new Set(sourceEntryIds.flatMap((entryId) => entryTeamIdsById.get(entryId) ?? [])));
    if (typeof divisions.update === 'function') {
      await divisions.update({
        where: { id: phase.id },
        data: { teamIds: participantTeamIds, updatedAt: new Date() },
      });
    }
    for (const eventTeamId of participantTeamIds) {
      const sourceEntryDivisionId = sourceEntryIds.find((entryId) => (
        (entryTeamIdsById.get(entryId) ?? []).includes(eventTeamId)
      )) ?? sourceEntryIds[0] ?? null;
      const participantId = `phase-participant-${eventId}-${phase.id}-${eventTeamId}`;
      await participants.upsert({
        where: { id: participantId },
        create: {
          id: participantId,
          eventId,
          phaseDivisionId: phase.id,
          eventTeamId,
