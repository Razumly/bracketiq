...
const liveApprovalCandidateFrom = (
  job: AffiliateMappingApprovalJob,
): AffiliateMappingLiveApprovalCandidate | null => {
  if (job.status !== 'REVIEW_REQUIRED') return null;
  ...
  const setupPaths = result.generatedPaths.filter((generatedPath) => (
    /^apps\/site\/scripts\/setup-[a-z0-9-]+-affiliate-source\.ts$/.test(generatedPath)
  ));
...
export const selectAffiliateMappingLiveApprovalCandidates = (
  ...
  for (const job of jobs) {
    const candidate = liveApprovalCandidateFrom(job);
    if (candidate === null) continue;
    if (candidate.result.logoDisposition === 'MANUAL_REVIEW') {
