...
export const renderAffiliateSourceDraft = (
  ...
  const configPath = `apps/site/src/server/affiliateImports/generatedSources/${configName}.ts`;
  const setupPath = `apps/site/scripts/setup-${slug}-affiliate-source.ts`;
  const testPath = `apps/site/src/server/affiliateImports/__tests__/${configName}.test.ts`;
