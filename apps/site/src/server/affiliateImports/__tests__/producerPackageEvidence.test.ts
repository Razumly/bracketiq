...
const result = (commit: string): CodexAffiliateIngestionResult => ({
  ...
  commit,
  generatedPaths: [
    'apps/site/scripts/setup-example-affiliate-source.ts',
    'apps/site/src/server/affiliateImports/example.ts',
  ],
...
const createRepository = () => {
  ...
  fs.mkdirSync(path.join(root, 'apps/site/scripts'), { recursive: true });
  fs.mkdirSync(path.join(root, 'apps/site/src/server/affiliateImports'), { recursive: true });
  fs.mkdirSync(path.join(root, 'node_modules'), { recursive: true });
  fs.writeFileSync(path.join(root, 'apps/site/scripts/setup-example-affiliate-source.ts'), 'export const version = 1;\n');
  fs.writeFileSync(path.join(root, 'apps/site/src/server/affiliateImports/example.ts'), 'export const example = true;\n');
  execFileSync('git', ['init', '-b', 'codex/affiliate-ingestion-live'], { cwd: root });
  execFileSync('git', ['config', 'user.name', 'Test'], { cwd: root });
  execFileSync('git', ['config', 'user.email', 'test@example.com'], { cwd: root });
  execFileSync('git', ['add', 'apps/site'], { cwd: root });
...
describe('affiliate producer package evidence', () => {
  ...
  it('verifies generated files in the exact commit and materializes that commit after HEAD advances', () => {
    ...
    try {
      ...
      expect(evidence.setupScript).toBe('apps/site/scripts/setup-example-affiliate-source.ts');
      expect(Object.keys(evidence.fileSha256)).toHaveLength(2);

      fs.writeFileSync(
        path.join(repository.root, 'apps/site/scripts/setup-example-affiliate-source.ts'),
        'export const version = 2;\n',
      );
      execFileSync('git', ['add', 'apps/site/scripts/setup-example-affiliate-source.ts'], { cwd: repository.root });
      ...
      try {
        ...
        expect(fs.lstatSync(path.join(snapshot.repositoryRoot, 'apps/site/node_modules')).isSymbolicLink()).toBe(true);
      } finally {
        snapshot.cleanup();
      }
    } finally {
