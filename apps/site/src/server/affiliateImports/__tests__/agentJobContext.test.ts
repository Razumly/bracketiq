...
describe('affiliate mapping job context builder', () => {
  it('verifies the export and retrieves bounded Markdown, HTML, policy, and repository context', async () => {
    ...
    try {
      ...
      await fs.mkdir(path.join(repositoryRoot, 'apps/site/src/server/affiliateImports'), { recursive: true });
      ...
      await Promise.all([
        ...
        fs.writeFile(
          path.join(repositoryRoot, 'apps/site/src/server/affiliateImports/types.ts'),
      ...
      expect(result.context).toEqual(expect.objectContaining({
        ...
        repositoryExcerpts: expect.arrayContaining([
          expect.objectContaining({ path: 'apps/site/src/server/affiliateImports/types.ts' }),
        ]),
      }));
    } finally {
  ...
  it('combines bounded evidence from multiple capture runs with exact provenance', async () => {
    ...
    try {
      await Promise.all([
        ...
        fs.mkdir(path.join(repositoryRoot, 'apps/site/src/server/affiliateImports'), { recursive: true }),
