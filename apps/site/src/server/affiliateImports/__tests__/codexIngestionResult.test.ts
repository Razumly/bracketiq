...
const successfulResult = {
  ...
  commit: 'b'.repeat(40),
  generatedPaths: [
    'apps/site/scripts/setup-river-city-affiliate-source.ts',
    'apps/site/src/server/affiliateImports/__tests__/riverCitySource.test.ts',
  ],
...
describe('Codex affiliate ingestion result', () => {
  ...
  it('accepts an unsupported-sport human-review result without package artifacts', () => {
    ...
    expect(() => codexAffiliateIngestionResultSchema.parse({
      ...humanReview,
      generatedPaths: ['apps/site/scripts/setup-river-city-affiliate-source.ts'],
