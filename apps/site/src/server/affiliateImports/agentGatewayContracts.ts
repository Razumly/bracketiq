...
const MAPPING_SPORT_EVIDENCE_INSTRUCTIONS = [
  "Use only the injected sportsCatalog and claim-owned first-party artifacts. Read complete relevant Markdown and needed HTML/images before reporting a gap. Cite the activity's surface or format; labels need not match catalog wording. Indoor/gym/hard-court volleyball supports Indoor Volleyball; sand/beach supports Beach Volleyball; grass/outdoor-field supports Grass Volleyball. A club's volleyball open gym is gym evidence; cite the activity-to-club link. A cancellation or schedule change can establish the setting, not an active session. Indoor hardwood supports Indoor Volleyball only with a cited volleyball-to-venue link. Venue name, city, URL, and database values alone are not proof. Read program and About prose before gallery text. An approved scope removes named activities only; it supplies no facts about retained sports.",
...
const SOURCE_ENTITY_INSTRUCTION =
  "Establish the entity from primary content and its action. A CLUB is one named club or organization; an EVENT is the described activity; a RENTAL is the offered space. " +
  "Treat sourceDocumentKind and article metadata as signals, not a kind decision. A dedicated page can qualify when its prose and action support that entity. " +
  "A news article or directory about several organizations is not one club; its publisher is not those organizations.";
...
> = {
  ...
  SUPPLY_REVIEWER: [
    ...
    "For package review, require a valid current entityActionQuality report, then independently check entity identity and action purpose against source evidence. Request producer repair for a fixable selector or wrong link. Use human review when the primary content does not support the claimed entity. Unresolved or unsupported sport evidence and unauthenticated user decisions still prevent approval.",
