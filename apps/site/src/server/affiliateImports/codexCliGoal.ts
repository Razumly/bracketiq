...
export const buildCodexAffiliateIngestionObjective = (
  ...
  return [
    ...
    'For repairContext, fix every repairReason and blockingIssue in a new commit, add a source regression test, and Add one generalized rule there when missing.',
    'For event-datetime-v1, inspect each occurrence; classify SCHEDULED, DATE_ONLY, NO_FIXED_DATE, or ONGOING. Verify timezone, precision, end, duration, DST, title-clock consistency, and UTC/non-UTC output. Evergreen rows need source-backed schedule text, no hidden dated session, and no tryouts/evaluations. Record transitions and repair codes.',
    ...
    'A REVIEW_REQUIRED event-datetime-v1 result needs dateTimeReview: contractRevision, candidateCount, timezone/precision/end counts, durationWarnings, utcHostRegression, displayModeCounts, evergreenTransitions, and evidence.',
    'For source-only work, run focused Jest, targeted ESLint, and scoped diffs. Never run full-project npx tsc --noEmit except for explicitly authorized shared importer or app changes.',
    'Use exact current Sports.name values. Never infer a volleyball or soccer surface.',
    'Treat unsupported source sport labels as human-review evidence: complete as HUMAN_REVIEW_REQUIRED with SPORT_NOT_IN_CATALOG and write no candidates.',
    'Run logo-fit only for the current organization; never --all.',
    'Treat each source event as one parent; group every division by its stable event identity/detail-page context. Preserve the exact source division label; use canonical gender M/F/C, ratingType AGE/SKILL. Every accepted EVENT must have a source-supported division. Exclude invalid events. Keep price/capacity on its division; differing division prices must produce a compact event range.',
