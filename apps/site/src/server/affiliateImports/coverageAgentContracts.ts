...
export const affiliateCoverageCampaignProposalSchema = z.object({
  ...
  coverageCellIds: z.array(z.string().trim().min(1)).max(20).default([]),
  strategyKeys: z.array(z.string().trim().min(1)).max(3).default([]),
...
export const affiliateCoverageCompletionSchema = z.object({
  ...
  coverageEvidence: z.object({
    cellIds: z.array(z.string().trim().min(1)).max(20).default([]),
    cycleKey: z.string().trim().min(1).nullable().default(null),
    ...
    strategyKeys: z.array(z.string().trim().min(1)).max(3).default([]),
    strategyFamilyKeys: z.array(z.string().trim().min(1)).max(20).default([]),
    successfulQueryExecutionIds: z.array(z.string().trim().min(1)).max(100).default([]),
    failedQueryExecutionIds: z.array(z.string().trim().min(1)).max(100).default([]),
    recentNewDomainYields: z.array(z.number().int().min(0)).max(20).default([]),
    newQualifiedPolicyKeyCount: z.number().int().min(0).default(0),
    unresolvedLeadCount: z.number().int().min(0).nullable().default(null),
    proposedNextReviewAt: z.coerce.date().nullable().default(null),
