...
export const codexAffiliateIngestionResultV2Schema =
  codexAffiliateIngestionResultV2BaseSchema.superRefine((result, context) => {
    ...
    if (result.status === 'HUMAN_REVIEW_REQUIRED') {
      const humanReview = result.humanReviewRequired;
      if (!humanReview) {
        addIssue(['humanReviewRequired'], 'HUMAN_REVIEW_REQUIRED results require a structured human-review reason.');
        return;
      }
      const hasSportReview = hasSportReasonCode(humanReview.reasonCodes);
      if (hasSportReview) {
        try {
          assertAffiliateSportDeterminationReasonStatusConsistency({
    ...
    if (result.humanReviewRequired) {
      addIssue(['humanReviewRequired'], 'Structured human-review reasons are only valid for HUMAN_REVIEW_REQUIRED results.');
    }
    if (result.dateTimeReview && result.dateTimeReview.candidateCount !== result.candidateCount) {
      addIssue(['dateTimeReview', 'candidateCount'], 'Datetime review candidateCount must match the ingestion result.');
    }
    if (result.status !== 'REVIEW_REQUIRED' && result.dateTimeReview) {
      addIssue(['dateTimeReview'], 'Datetime review evidence is only valid for REVIEW_REQUIRED mapping results.');
    }
    if (result.status === 'FAILED') {
      if (!result.errorMessage) addIssue(['errorMessage'], 'FAILED ingestion results require an error message.');
      return;
    }
    if (result.status === 'EXPANDED') {
      const expansion = result.directoryExpansion;
      if (!expansion) {
        addIssue(['directoryExpansion'], 'EXPANDED ingestion results require a directory expansion summary.');
      } else if (
        expansion.created + expansion.reused + expansion.duplicate + expansion.rejected !== expansion.submitted
      ) {
        addIssue(['directoryExpansion'], 'Directory expansion outcomes must account for every submitted URL.');
      }
      if (
        result.branch
        || result.commit
        ...
        addIssue(['status'], 'EXPANDED ingestion results cannot claim mapping artifacts, candidates, scrapes, commits, or errors.');
      }
      return;
    }

    if (result.directoryExpansion) {
      addIssue(['directoryExpansion'], 'Directory expansion summaries are only valid for EXPANDED results.');
    }
    if (!result.commit) addIssue(['commit'], 'REVIEW_REQUIRED ingestion results require a source-scoped commit.');
    if (!result.branch) addIssue(['branch'], 'REVIEW_REQUIRED ingestion results require the source branch.');
    if (!result.generatedPaths.length) {
      addIssue(['generatedPaths'], 'REVIEW_REQUIRED ingestion results require generated source package paths.');
    }
    if (!result.validation.testsPassed || !result.validation.diffCheckPassed || !result.validation.duplicateSafe) {
      addIssue(['validation'], 'REVIEW_REQUIRED ingestion results require passing tests, diff, and duplicate checks.');
    }
    if (result.reviewScrapes.length !== 2 || result.reviewScrapes.some((scrape) => !scrape.passed)) {
      addIssue(['reviewScrapes'], 'REVIEW_REQUIRED ingestion results require exactly two passing review scrapes.');
    }
    if (
      result.reviewScrapes.length === 2
      && (
      ...
      addIssue(['reviewScrapes'], 'Review scrapes must have stable counts and normalized candidate hashes.');
    }
    if (result.errorMessage) addIssue(['errorMessage'], 'REVIEW_REQUIRED ingestion results cannot include an error message.');
    try {
      ...
      addIssue(['sportDeterminations'], error instanceof Error ? error.message : 'Invalid review-ready determinations.');
    }
  });
