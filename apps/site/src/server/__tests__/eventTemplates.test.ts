...
describe('dedicated event templates', () => {
  it('stores reusable event parameters, slot offsets, and rental resource hints without live event state', () => {
    ...
    expect(bundle.template).not.toHaveProperty('officialSchedulingMode');
  ...
  it('maps legacy source-event staffing modes at the intake boundary without leaking them', () => {
    const legacyCases = [
    ...
    for (const [officialSchedulingMode, staffingPriority] of legacyCases) {
      const bundle = mapSourceEventToTemplateBundle(
        buildSourceEvent({
          staffingPriority: undefined,
          officialSchedulingMode,
        }),
        {
          templateId: `template_${officialSchedulingMode.toLowerCase()}`,
          createdByUserId: 'host_1',
        },
      );

      expect(bundle.template.staffingPriority).toBe(staffingPriority);
      expect(bundle.template).not.toHaveProperty('officialSchedulingMode');
    }
  });

  it('normalizes legacy stored template rows at read boundaries without leaking the old field', async () => {
    const storedTemplate = {
      id: 'template_legacy',
      name: 'Legacy template',
      description: null,
      sourceEventId: 'event_legacy',
      ownerUserId: 'host_1',
      organizationId: null,
      sportIds: ['sport_1'],
      eventType: 'EVENT',
    ...
    const templateClient = {
      eventTemplates: {
        findUnique: jest.fn().mockResolvedValue(storedTemplate),
        findMany: jest.fn().mockResolvedValue([storedTemplate]),
      },
    ...
    // The focused fake implements the template delegates used by these read paths.
    const client = templateClient as unknown as NonNullable<
      Parameters<typeof getEventTemplate>[1]
    >;

    const bundle = await getEventTemplate('template_legacy', client);
    const [summary] = await listEventTemplates({}, 10, client);

    expect(bundle?.template.staffingPriority).toBe('TEAM_COVERAGE_REQUIRED');
    expect(bundle?.template).not.toHaveProperty('officialSchedulingMode');
    expect(summary.staffingPriority).toBe('TEAM_COVERAGE_REQUIRED');
    expect(summary).not.toHaveProperty('officialSchedulingMode');
  });
