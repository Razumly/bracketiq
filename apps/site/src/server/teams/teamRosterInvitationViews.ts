...
import { expireTeamInvitations, isInvitationExpired } from './teamInvitationState';
...
export const withRosterInvitationViews = async <T extends RosterView>(client: Prisma.TransactionClient, teams: T[]): Promise<T[]> => {
  ...
  const latest = await client.invites.findMany({
    ...
    orderBy: [{ createdAt: { sort: 'desc', nulls: 'last' } }, { id: 'desc' }], select: { id: true, teamId: true, userId: true, status: true, linkExpiresAt: true },
  });
  ...
  return teams.map((team) => ({ ...team, playerRegistrations: registrationsFor(team).map((registration) => {
    ...
    return !invite ? registration : { ...registration, invitationId: invite.id,
      invitationLabel: invite.status === 'EXPIRED' || isInvitationExpired(invite) ? 'Invitation expired' : `Invitation ${String(invite.status ?? 'pending').toLowerCase()}` };
  }) }));
};
