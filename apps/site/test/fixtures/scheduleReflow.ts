...
export function createCanonicalReflowFixture(id: string, assignmentOnly = false) {
  const division = new Division(`${id}:division`, 'Open');
  division.phase = 'BRACKET';
  division.phaseSettings = { BRACKET: { doTeamsOfficiate: assignmentOnly } };
  ...
  const first = new Match({ id: `${id}:1`, matchId: 1, eventId: id, field, division, bufferMs: 300_000,
    start: reflowFixtureTime(0), end: reflowFixtureTime(25), actualEnd: reflowFixtureTime(assignmentOnly ? 25 : 35),
  ...
  const next = new Match({ id: `${id}:2`, matchId: 2, eventId: id, field, division, bufferMs: 300_000,
    ...
    team1: teams[0], team2: teams[2], teamOfficial: assignmentOnly ? teams[0] : null });
  ...
  const event = new Tournament({ id, name: 'Harbor Cup', eventType: 'TOURNAMENT', hostId: `${id}:host`,
    ...
    restTimeMinutes: 5, doTeamsOfficiate: assignmentOnly,
    staffingPriority: assignmentOnly ? 'FULL_COVERAGE_REQUIRED' : 'BEST_AVAILABLE_COVERAGE' });
