...
test("joins the seeded free event via self registration", async ({ page }) => {
  ...
  const registrationRequestPromise = page.waitForRequest(
    (req) =>
      req
        .url()
        .includes(`/api/events/${SEED_EVENTS.free.id}/registrations/self`) &&
      req.method() === "POST",
  );
  ...
  const participantsRequest = await page
    .waitForRequest(
      (req) =>
        req.url().includes(`/api/events/${SEED_EVENTS.free.id}/participants`) &&
        req.method() === "POST",
      { timeout: 2000 },
    )
    .catch(() => null);
  ...
  if (participantsRequest) {
    const participantsPayload = participantsRequest.postDataJSON() as {
      user?: { $id?: string; id?: string };
    };
    const userId =
      participantsPayload.user?.$id ?? participantsPayload.user?.id;
...
test("joins the seeded paid event and asserts payment intent payload", async ({
  page,
}) => {
  await seedLocationStorage(page);
  await ensureDobVerified(page.request, SEED_USERS.participant.id);
  ...
  const purchaseEventId =
    purchasePayload.event?.$id ?? purchasePayload.event?.id;
