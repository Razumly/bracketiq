...
Each mounted Git workspace must be a checkout of `Razumly/bracketiq` at the monorepo root. The existing host directory names can remain unchanged. Compose mounts each repository at `/workspace`, runs commands from `/workspace/apps/site`, and mounts the environment file at `/workspace/apps/site/.env.local`. Producer roots under `/producer-workspaces` also identify monorepo roots.

After the repository cutover and while the fleet is stopped, update every checkout to the same BracketIQ commit. Run `npm ci` from `apps/site` in the primary mapper, approval, and coverage checkouts. Mapper 2 through mapper 10 use the primary mapper's `apps/site/node_modules` as a read-only mount. Confirm that every checkout contains `apps/site/package.json` before container creation.
