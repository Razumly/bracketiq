...
if [[ -z "$new_image" ]]; then
  echo "Usage: $0 ghcr.io/razumly/bracketiq-site:<full-commit-sha>" >&2
  exit 64
fi

if [[ "$new_image" =~ ^ghcr\.io/razumly/bracketiq-site:([0-9a-f]{40})$ ]]; then
  commit_sha="${BASH_REMATCH[1]}"
elif [[ "$new_image" =~ ^ghcr\.io/razumly/bracketiq-site@sha256:[0-9a-f]{64}$ ]]; then
