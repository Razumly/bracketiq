...
const subscribeToObsBridge = (onStoreChange: () => void) => {
...
const getObsBridgeAvailable = () =>
  typeof window !== "undefined" &&
  typeof window.obsstudio?.saveReplayBuffer === "function";
const getServerObsBridgeAvailable = () => false;
...
const normalizeManualScore = (
  ...
  const fallbackTarget =
    competition.setTargets[competition.setTargets.length - 1] ?? 21;
...
export default function BroadcastControlRoom({
  ...
  const setManualCurrentSet = (value: string | number) => {
    const nextCurrentSet = Math.max(
      1,
      Math.min(presentation.competition.bestOf, asNonNegativeInteger(value)),
    );
    setManualScore((current) => {
      const normalized = normalizeManualScore(
        current,
        presentation.competition,
      );
      const selectedSet = normalized.sets.find(
        (set) => set.sequence === nextCurrentSet,
      );
      return {
        ...normalized,
        currentSet: nextCurrentSet,
        points: selectedSet
          ? [selectedSet.team1Points, selectedSet.team2Points]
          : [0, 0],
      };
    });
  };

  const updateManualSetScore = (
    sequence: number,
    teamIndex: 0 | 1,
    value: string | number,
  ) => {
    ...
    setManualScore((current) => {
      ...
      const selectedSet = sets.find(
        (set) => set.sequence === normalized.currentSet,
      );
      return {
        ...normalized,
        sets,
  ...
  const updateManualCurrentPoints = (
    teamIndex: 0 | 1,
    value: string | number,
  ) => {
    const points = asNonNegativeInteger(value);
  ...
  return (
    <Paper withBorder p="md" radius="md">
      <Group justify="space-between" mb="sm">
        ...
        <Group gap="xs">
          <Badge color={manualOverrideActive ? "orange" : "blue"}>
            {state.scoringMode}
          </Badge>
      ...
      <Stack gap="sm">
        <Group grow>
          <Button
            ...
            disabled={disabled}
            variant={
              presentation.presentation.scoreboardVisible ? "filled" : "default"
            }
            onClick={() =>
              void onCommand({
                type: "SET_VISIBILITY",
                visible: !presentation.presentation.scoreboardVisible,
              })
            }
          >
            {presentation.presentation.scoreboardVisible
              ? "Hide scorebug"
              : "Show scorebug"}
          </Button>
          <Button
            ...
            onClick={() =>
              void onCommand({
                type: presentation.presentation.activeStinger
                  ? "HIDE_STINGER"
                  : "SHOW_STINGER",
                ...(presentation.presentation.activeStinger
                  ? {}
                  : { stinger: "MATCH_INTRO" }),
              })
            }
        ...
        <SimpleGrid cols={{ base: 2, sm: 4 }}>
          {BROADCAST_STINGER_KINDS.map((stinger) => (
            <Button
              ...
              disabled={disabled}
              variant={
                presentation.presentation.activeStinger === stinger
                  ? "filled"
                  : "light"
              }
        ...
        {obsNotice ? (
          <Alert
            color={obsNotice.startsWith("OBS accepted") ? "green" : "gray"}
          >
            {obsNotice}
          </Alert>
        ) : null}
        ...
        {!manualOverrideActive ? (
          <Button
            ...
            onClick={() => {
              if (
                window.confirm(
                  "Enter manual presentation override? This never changes official Match or MatchSegment records.",
                )
              ) {
                void onCommand({
        ...
        <Text size="xs" c="dimmed">
          These controls write presentation state only. They never modify
          official Match or MatchSegment scores.
        </Text>
      </Stack>
    </Paper>
  );
