...
const LEGACY_SPORT_ID_ALIASES: Record<string, string> = {
...
class EventService {
  private resolveSportFromMap(
    ...
    const exact = sportsMap.get(normalizedSportId);
    if (exact) {
      return exact;
    }

    const legacyAlias =
      LEGACY_SPORT_ID_ALIASES[normalizedSportId.toLowerCase()];
    if (legacyAlias) {
      const aliased = sportsMap.get(legacyAlias);
      if (aliased) {
        return aliased;
      }
    }

    const normalizedLookup = normalizedSportId.toLowerCase();
    for (const sport of sportsMap.values()) {
      const sportIdValue =
        typeof sport.$id === "string" ? sport.$id.trim().toLowerCase() : "";
      const sportNameValue =
        typeof sport.name === "string" ? sport.name.trim().toLowerCase() : "";
      if (
        sportIdValue === normalizedLookup ||
        sportNameValue === normalizedLookup
      ) {
        return sport;
      }
    }
  ...
  private normalizeMatchOfficialAssignments(
    value: unknown,
  ): MatchOfficialAssignment[] {
    if (!Array.isArray(value)) {
      return [];
    }
    const normalized: Array<MatchOfficialAssignment | null> = value.map(
      (entry): MatchOfficialAssignment | null => {
    ...
    return normalized.filter(
      (entry): entry is MatchOfficialAssignment => entry !== null,
    );
  }
    ...
    return {
      ...
      participantCount:
        typeof response?.participantCount === "number"
          ...
          : 0,
      participantCapacity:
        typeof response?.participantCapacity === "number"
  ...
  async deleteEventResult(event: Event): Promise<DeleteOrArchiveResult> {
    try {
      const normalizedEvent = this.withNormalizedOfficials(event);
  ...
  private mapEventOfficialPositions(
    value: unknown,
  ): NonNullable<Event["officialPositions"]> {
    if (!Array.isArray(value)) {
      return [];
    }
    return value
      ...
      .filter(
        (entry): entry is NonNullable<Event["officialPositions"]>[number] =>
          Boolean(entry),
      )
  ...
  private mapEventOfficials(
            ...
            ? Array.from(
                new Set(
                  row.positionIds
                    ...
                    .filter(
                      (positionId) =>
                        positionId.length > 0 && positionIdSet.has(positionId),
                    ),
                ),
              )
            : [...positionIds],
    ...
    return officialIds.map((userId) => {
      ...
      if (existing) {
        return {
          ...existing,
          positionIds: existing.positionIds.length
    ...
    return {
            ...
            .map((entry: any) => ({
              ...
              kind:
                entry?.kind === "LEAGUE" || entry?.kind === "PLAYOFF"
                  ...
                  : "LEAGUE",
              divisionTypeId:
                typeof entry?.divisionTypeId === "string"
                  ? entry.divisionTypeId
                  : undefined,
              divisionTypeName:
                typeof entry?.divisionTypeName === "string"
                  ? entry.divisionTypeName
                  : undefined,
              ratingType:
              ...
              gender:
                ...
                entry?.gender === "C"
                  ...
                  : undefined,
              sportId:
                typeof entry?.sportId === "string" ? entry.sportId : undefined,
              price:
              ...
              maxParticipants:
                typeof entry?.maxParticipants === "number"
                  ...
                  : Number.isFinite(Number(entry?.maxParticipants))
                    ...
                    : undefined,
              playoffTeamCount:
                typeof entry?.playoffTeamCount === "number"
                  ? entry.playoffTeamCount
                  : Number.isFinite(Number(entry?.playoffTeamCount))
                    ? Number(entry.playoffTeamCount)
                    : undefined,
              playoffPlacementDivisionIds: Array.isArray(
                entry?.playoffPlacementDivisionIds,
              )
                ? entry.playoffPlacementDivisionIds.map((divisionId: unknown) =>
              ...
              allowPaymentPlans:
                typeof entry?.allowPaymentPlans === "boolean"
                  ...
                  : undefined,
              installmentCount:
                typeof entry?.installmentCount === "number"
              ...
              fieldIds: Array.isArray(entry?.fieldIds)
                ? entry.fieldIds
                    .map((fieldId: unknown) => String(fieldId))
                    .filter(Boolean)
                : [],
              teamIds: Array.isArray(entry?.teamIds)
                ? entry.teamIds
                    .filter(
                      (teamId: unknown): teamId is string =>
                ...
                : [],
              ageCutoffDate:
                typeof entry?.ageCutoffDate === "string"
                  ? entry.ageCutoffDate
                  : undefined,
              ageCutoffLabel:
                typeof entry?.ageCutoffLabel === "string"
                  ? entry.ageCutoffLabel
                  : undefined,
              ageCutoffSource:
                typeof entry?.ageCutoffSource === "string"
                  ? entry.ageCutoffSource
                  : undefined,
            }))
            .filter((entry: any) => entry.id.length > 0)
        : undefined,
      playoffDivisionDetails: Array.isArray(row.playoffDivisionDetails)
        ? row.playoffDivisionDetails
            .map((entry: any) => ({
              id: String(entry?.id ?? entry?.$id ?? ""),
              name: String(entry?.name ?? entry?.id ?? ""),
              key: typeof entry?.key === "string" ? entry.key : undefined,
      ...
      divisionFieldIds:
        row.divisionFieldIds && typeof row.divisionFieldIds === "object"
          ? Object.fromEntries(
              Object.entries(
                row.divisionFieldIds as Record<string, unknown>,
              ).map(([key, value]) => [
                key,
                Array.isArray(value)
      ...
      officials: Array.isArray(row.officials)
        ? (row.officials as UserData[])
        : undefined,
      assistantHosts: Array.isArray(row.assistantHosts)
        ? (row.assistantHosts as UserData[])
        : undefined,
      ...
      participantCount:
        typeof row.participantCount === "number"
          ...
          : Number.isFinite(Number(row.participantCount))
            ...
            : null,
      participantCapacity:
        typeof row.participantCapacity === "number"
  ...
  async getBlockingForFieldInRange(
    ...
    const excludeEventId =
      typeof options?.excludeEventId === "string"
        ? options.excludeEventId.trim()
        : "";
    ...
    const response = await apiRequest<{ events?: any[]; rentalSlots?: any[] }>(
      `/api/events/field/${fieldId}?${params.toString()}`,
    );
    const rows = Array.isArray(response?.events) ? response.events : [];
    const rentalRows = Array.isArray(response?.rentalSlots)
      ? response.rentalSlots
      : [];
      ...
      ? rows.map(
          (row) =>
            ({
              ...
              parentEvent:
                typeof row.parentEvent === "string" ? row.parentEvent : null,
    ...
    if (!options?.rentalOverlapOnly) {
      const allTimeSlotIds = this.extractStringIds(
        events.flatMap((event) => event.timeSlotIds ?? []),
      );
  ...
  async getEventsForFieldInRange(
    ...
    const result = await this.getBlockingForFieldInRange(
      fieldId,
      start,
      end,
      options,
  ...
  async getMatchesForFieldInRange(
    ...
    const response = await apiRequest<{ matches?: any[] }>(
      `/api/fields/${fieldId}/matches?${params.toString()}`,
    );
  ...
  private async hydrateEventRelations(
    ...
    const fieldIds = this.extractStringIds(
      data.fieldIds ?? event.fieldIds ?? [],
    );
    const timeSlotIds = this.extractStringIds(
    ...
    const [
      ...
      this.resolveOrganization(
        data.organization ?? data.organizationId ?? event.organization,
      ),
  ...
  private async fetchTeamsByIds(ids: string[]): Promise<Team[]> {
    ...
    return responses.flatMap((response) =>
      (response?.teams ?? []).map((row: any) => this.mapTeamRow(row)),
    );
  }
  ...
  private mapTeamRow(row: any): Team {
    ...
    if (
      row.division &&
      typeof row.division === "object" &&
      ("name" in row.division || "id" in row.division)
    ) {
    ...
    const team: Team = {
      ...
      division,
      divisionTypeId:
        typeof row.divisionTypeId === "string" &&
      ...
      coachIds: Array.isArray(row.assistantCoachIds)
        ...
        : Array.isArray(row.coachIds)
          ...
          : [],
      parentTeamId:
        typeof row.parentTeamId === "string" &&
        row.parentTeamId.trim().length > 0
          ? row.parentTeamId
          : null,
      pending,
      playerRegistrations,
  ...
  private async fetchFieldsByIds(ids: string[]): Promise<Field[]> {
    ...
    const responses = await Promise.all(
      this.chunkIds(unique).map((batch) => {
        const params = new URLSearchParams();
        params.set("ids", batch.join(","));
        return apiRequest<{ fields?: any[] }>(
          `/api/fields?${params.toString()}`,
        );
      }),
    );

    const fields = responses.flatMap((response) =>
      (response?.fields ?? []).map((row: any) => this.mapFieldRow(row)),
    );
    await this.hydrateFieldRentalSlots(fields);
    return fields;
  }
  ...
  private async hydrateFieldRentalSlots(fields: Field[]): Promise<void> {
    const allIds = Array.from(
      new Set(
        fields.flatMap((field) =>
          Array.isArray(field.rentalSlotIds) ? field.rentalSlotIds : [],
        ),
      ),
    );
    ...
    fields.forEach((field) => {
      ...
      field.rentalSlots = ids
        .map((id) => slotMap.get(id))
        .filter((slot): slot is TimeSlot => Boolean(slot));
    });
  }

  private async resolveTimeSlots(
    existing: any,
    ids: string[],
  ): Promise<TimeSlot[]> {
    if (Array.isArray(existing) && existing.length) {
  ...
  private async fetchTimeSlotsByIds(ids: string[]): Promise<TimeSlot[]> {
    ...
    const responses = await Promise.all(
      this.chunkIds(unique).map((batch) => {
        const params = new URLSearchParams();
        params.set("ids", batch.join(","));
        return apiRequest<{ timeSlots?: any[] }>(
    ...
    return responses.flatMap((response) =>
      (response?.timeSlots ?? []).map((row: any) => this.mapRowToTimeSlot(row)),
    );
  }
  ...
  private async resolveMatches(
    ...
    const rows =
      Array.isArray(existing) && existing.length
        ? existing
        : await this.fetchMatchesByEventId(event.$id);
    if (!rows.length) {
      return [];
    }
    ...
    rows.forEach((row) => {
      ...
      this.normalizeMatchOfficialAssignments(row.officialIds).forEach(
        (assignment) => {
    ...
    const missingTeamOfficialIds = Array.from(teamOfficialIds).filter(
      (id) => !teamsById.has(id),
    );
    if (missingTeamOfficialIds.length) {
      const fetchedTeamOfficials = await this.fetchTeamsByIds(
        missingTeamOfficialIds,
      );
      fetchedTeamOfficials.forEach((team) => teamsById.set(team.$id, team));
    }

    const missingOfficialIds = Array.from(officialIds).filter(
      (id) => !officialsById.has(id),
    );
    ...
    return (rows as any[]).map((row) => {
      const match = this.mapMatchRecord(row, {
        teamsById,
        fieldsById,
        officialsById,
      });
  ...
  private mapOrganizationRow(row: any): Organization {
    const coordinates = Array.isArray(row.coordinates)
      ? (row.coordinates.slice(0, 2).map((value: unknown) => Number(value)) as [
          number,
          number,
        ])
      : undefined;
    ...
    const organization: Organization = {
      ...
      verificationReviewStatus:
        typeof row.verificationReviewStatus === "string"
          ? row.verificationReviewStatus
          : undefined,
      verificationReviewNotes:
        typeof row.verificationReviewNotes === "string"
          ? row.verificationReviewNotes
          : undefined,
      verificationReviewUpdatedAt:
        typeof row.verificationReviewUpdatedAt === "string"
  ...
  private mapMatchRecord(
    ...
    context?: {
      ...
      officialsById?: Map<string, UserData>;
    },
  ): Match {
    const eventId =
    ...
    if (
      !resolvedTeamOfficialId &&
      resolvedOfficialId &&
      context?.teamsById?.has(resolvedOfficialId)
    ) {
    ...
    const match: Match = {
      ...
      winnerNextMatchId:
        input.winnerNextMatchId ??
        (input.winnerNextMatch
          ? (input.winnerNextMatch as Match).$id
          : undefined),
      loserNextMatchId:
        input.loserNextMatchId ??
        (input.loserNextMatch
          ? (input.loserNextMatch as Match).$id
          : undefined),
    ...
    if (
      resolvedTeamOfficialId &&
      context?.teamsById?.has(resolvedTeamOfficialId)
    ) {
    ...
    if (
      match.teamOfficial &&
      (typeof match.teamOfficialSeed !== "number" ||
        !Number.isFinite(match.teamOfficialSeed))
    ) {
    ...
    match.officialCheckedIn =
      match.officialCheckedIn ?? input.officialCheckedIn ?? undefined;
  ...
  private mapRowToTimeSlot(row: any): TimeSlot {
    const startTime =
      this.normalizeTime(row.startTimeMinutes ?? row.startTime) ?? 0;
    const endTime =
      this.normalizeTime(row.endTimeMinutes ?? row.endTime) ?? startTime;
  ...
  private async ensureSportRelationship(row: any): Promise<void> {
    ...
    let sport = this.resolveSportFromMap(sportsMap, sportId);

    if (!sport) {
      sportsMap = await this.refreshSportsMap();
      sport = this.resolveSportFromMap(sportsMap, sportId);
    }
  ...
  private async getSportsMap(): Promise<Map<string, Sport>> {
    ...
    if (!this.sportsCachePromise) {
      this.sportsCachePromise = sportsService
        .getAll()
        .then((sports) => {
  ...
  ): Promise<{ hiddenEventIds: string[] }> {
    return apiRequest<{ hiddenEventIds?: string[] }>(
