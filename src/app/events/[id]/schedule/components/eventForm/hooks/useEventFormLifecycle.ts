...
export type EventFormLifecycleStabilizationState = {
  buildDraftForDirtyTrackingRef: MutableRefObject<
    (values: EventFormValues) => Partial<Event>
  >;
  dirtyBaselineValuesRef: MutableRefObject<EventFormValues | null>;
  onDirtyStateChange?: (hasChanges: boolean) => void;
  pendingInitialDirtyRebaseRef: MutableRefObject<boolean>;
  pendingInitialDirtyRebaseTimeoutRef: MutableRefObject<ReturnType<
    typeof setTimeout
  > | null>;
...
export const useEventFormDefaults = ({
  ...
  const immutableDefaultsMemo = useMemo(
    () => immutableDefaults ?? {},
    [immutableDefaults],
  );
  ...
  const immutableTimeSlotsFromDefaults = useMemo(
    () =>
      normalizeImmutableTimeSlots(
        immutableDefaultsMemo.timeSlots,
        immutableFields,
      ),
    [immutableDefaultsMemo.timeSlots, immutableFields],
  );
  const explicitlyImmutableFieldNames = useMemo(
    () =>
      Array.isArray(immutableDefaultsMemo.immutableFieldNames)
        ? new Set(immutableDefaultsMemo.immutableFieldNames)
        : null,
    [immutableDefaultsMemo.immutableFieldNames],
  );
  const isImmutableField = useCallback(
    (key: keyof Event) =>
      explicitlyImmutableFieldNames
        ? explicitlyImmutableFieldNames.has(key)
        : immutableDefaultsMemo[key] !== undefined,
    [explicitlyImmutableFieldNames, immutableDefaultsMemo],
  );
  const applyImmutableDefaults = useCallback(
...
export const useEventFormLifecycle = ({
  ...
  const pendingInitialDirtyRebaseTimeoutRef = useRef<ReturnType<
    typeof setTimeout
  > | null>(null);
  const buildDraftForDirtyTrackingRef = useRef<
    (values: EventFormValues) => Partial<Event>
  >(() => ({}));
  ...
  useEffect(() => {
    ...
    previousEventFieldLocationRef.current = defaultFieldLocationForEvent(
      nextDefaults.location,
    );
    dirtyBaselineValuesRef.current = null;
    reset(nextDefaults);
  }, [
...
export const useEventFormLifecycleStabilization = ({
  ...
  useEffect(() => {
    if (
      !open ||
      !pendingInitialDirtyRebaseRef.current ||
      sportsLoading ||
      fieldsLoading
    ) {
      if (pendingInitialDirtyRebaseTimeoutRef.current) {
    ...
    const expectedDraftFingerprint = JSON.stringify(
    ...
    pendingInitialDirtyRebaseTimeoutRef.current = setTimeout(() => {
      ...
      const latestDraftFingerprint = JSON.stringify(
        buildDraftEvent(getValues()),
      );
