...
const buildLifecycleEvent = (overrides: Partial<Event> = {}): Event =>
  buildEvent({
    $id: "event_1",
    name: "Initial Event",
...
describe("useEventFormLifecycle", () => {
  ...
  it("uses explicit immutable field names instead of locking every snapshot value", () => {
    const event = buildLifecycleEvent();
    const { result } = renderHook(() =>
      useLifecycleHarness({
        event,
        immutableDefaults: {
          immutableFieldNames: ["eventType"],
          noFixedEndDateTime: false,
        } as Partial<Event>,
      }),
    );

    expect(result.current.isImmutableField("eventType")).toBe(true);
    expect(result.current.isImmutableField("noFixedEndDateTime")).toBe(false);
  });
  ...
  it("preserves local edits for the same source and resets when the event id changes", async () => {
    ...
    await waitFor(() =>
      expect(result.current.formValues.name).toBe("Different Event"),
    );
