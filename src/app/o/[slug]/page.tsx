...
const getTeamCapacityFill = (team: PublicOrganizationTeamCard): number =>
  team.teamSize > 0
    ? Math.max(
        0,
        Math.min(100, Math.round((team.currentSize / team.teamSize) * 100)),
      )
    : 0;
  ...
  return (
    <Link href={event.detailsUrl} className={styles.item}>
      ...
      <div className={styles.itemBody}>
        ...
        <p className={styles.itemMeta}>
          {event.sportNames?.join(", ") || event.sportName || event.eventType} -{" "}
...
function TeamItem({ team }: { team: PublicOrganizationTeamCard }) {
  ...
  const isAffiliateTeam =
    typeof team.affiliateUrl === "string" &&
    team.affiliateUrl.trim().length > 0;
    ...
    <>
      ...
      <div className={styles.itemBody}>
        ...
        <div className={styles.teamCapacity} aria-label={capacityLabel}>
          ...
          {team.teamSize > 0 ? (
            <span className={styles.teamCapacityTrack} aria-hidden="true">
              <span
                className={styles.teamCapacityFill}
        ...
        {team.registrationUrl ? (
          <span className={styles.itemAction}>{registrationLabel}</span>
        ) : null}
    ...
    <a href={rental.detailsUrl} className={styles.item}>
      <div className={styles.itemBody}>
        <h3 className={styles.itemTitle}>
          {rental.facilityName ?? rental.fieldName}
        </h3>
        {rental.facilityName ? (
          <p className={styles.itemMeta}>{rental.fieldName}</p>
        ) : null}
        <p className={styles.itemMeta}>
          {rental.location ?? rental.facilityLocation ?? "Location TBD"}
        </p>
        <p className={styles.itemMeta}>
          {formatDate(rental.start)} - {formatPrice(rental.priceCents)}
        </p>
...
function DivisionItem({
  ...
  division: PublicOrganizationDivisionCard;
}) {
  const content = (
    <>
      <h3 className={styles.divisionTitle}>{division.name}</h3>
      {division.divisionTypeName ? (
        <p className={styles.itemMeta}>{division.divisionTypeName}</p>
      ) : null}
      {division.description ? (
        <p className={styles.itemMeta}>{division.description}</p>
      ) : null}
...
export default async function PublicOrganizationPage({
  ...
  const reviewPayload = await getOrganizationReviewsPayload(
    organization.id,
    null,
    { limit: 6 },
  );
  ...
  return (
    <main className={styles.page} style={pageStyle}>
        ...
        <div className={styles.heroInner}>
          ...
          {reviewPayload.summary.reviewCount > 0 ? (
            <div
              ...
              aria-label={`${reviewPayload.summary.averageRating} out of 5 stars`}
            >
              <span className={styles.heroRatingValue}>
                {reviewPayload.summary.averageRating?.toFixed(1)}
              </span>
              <span className={styles.stars} aria-hidden="true">
                ★★★★★
              </span>
              <span>
                {reviewPayload.summary.reviewCount}{" "}
                {reviewPayload.summary.reviewCount === 1 ? "review" : "reviews"}
              </span>
            </div>
          ) : null}
          <div className={styles.heroActions}>
            ...
            <a href="#rentals" className={styles.buttonSecondary}>
              Book rentals
            </a>
          </div>
        </div>
      ...
      <div className={styles.content}>
        <section
          className={styles.ownershipPanel}
          ...
          <section id="divisions" className={styles.section}>
            <div className={styles.sectionHeader}>
              <div>
                ...
                <p className={styles.sectionText}>
                  Current organization divisions and registration options.
                </p>
              </div>
            </div>
            ...
            {divisions.length > 2 ? (
              <details className={styles.divisionMore}>
                <summary className={styles.sectionAction}>
                  More ({divisions.length - 2})
                </summary>
                <div className={styles.divisionGrid}>
        ...
        <section id="reviews" className={styles.section}>
          <div className={styles.sectionHeader}>
            <div>
              ...
              <p className={styles.sectionText}>
                Feedback from people in the BracketIQ community.
              </p>
            </div>
            <Link
              href={`/organizations/${encodeURIComponent(organization.id)}/reviews`}
        ...
        <section id="events" className={styles.section}>
          <div className={styles.sectionHeader}>
            <div>
              ...
              <p className={styles.sectionText}>
                Leagues, tournaments, weekly sessions, and open events.
              </p>
            </div>
          </div>
          {events.length ? (
        ...
        <section id="teams" className={styles.section}>
          <div className={styles.sectionHeader}>
            <div>
              ...
              <p className={styles.sectionText}>
                Active organization teams and roster groups.
              </p>
            </div>
          </div>
          {teams.length ? (
        ...
        <section id="rentals" className={styles.section}>
          <div className={styles.sectionHeader}>
            <div>
              ...
              <p className={styles.sectionText}>
                Available field and court reservations.
              </p>
            </div>
          </div>
          {rentals.length ? (
        ...
        <section id="products" className={styles.section}>
          <div className={styles.sectionHeader}>
            <div>
              ...
              <p className={styles.sectionText}>
                Memberships, passes, and organization products.
              </p>
            </div>
          </div>
          {products.length ? (
      ...
      <BlogStructuredData
        data={createPublicOrganizationStructuredData({ organization, events })}
      />
    </main>
  );
}
