...
The shared event controls must use `@/components/organization/organization-operation-ui` for buttons, text inputs, selects, multiselects, date inputs, papers, and layout. They may use `DivisionDiscoveryFilters` for Discover-only division filters.
...
Revision note (2026-09-10 17:37Z): Follow-up review moved Discover filter panels into the shared portaled popover primitives, reserved the More-control gap during width fitting, and pruned unavailable skill selections from the always-mounted Events state when sports change. The affected Discover suites passed 2 suites and 18 tests; TypeScript, changed-file complexity lint, and the production build passed. The existing runtime remained unchanged because no restart authorization was available.
Revision note (2026-09-10 17:53Z): Follow-up review scoped Discover color tokens to the portaled filter popup and made the managed popup the single named dialog. The production build and affected Discover suites passed again. The explicitly requested local runtime rerun succeeded; browser verification showed one portaled Event type dialog with a non-transparent surface and teal selected rows.

Revision note (2026-09-10 18:36Z): Replaced Discover Events mobile filter and sport sheets plus the Organizations, Rentals, and Teams side panels with shared measured sport and filter rows and portaled dropdowns. Active filters now render beside each result count. The updating-events status copy is removed. The search field joins its submit button. Focused Discover suites passed 2 suites and 32 tests. TypeScript and the production build passed. Changed-file lint reported 0 errors and 5 existing complexity warnings. The current runtime was not restarted, so the current bundle has no browser verification.

Revision note (2026-09-10 18:59Z): Reproduced the Price maximum focus loss with a forced overflow layout; typing `25` stopped at `2`. The overflow row now keeps its measured partition and live popup mounted while it is open, then remeasures after close. The regression and full affected Discover suites passed: 2 suites and 33 tests. TypeScript, changed-file lint, and the production build passed. The current runtime was not restarted.
Revision note (2026-09-10 19:48Z): Rebuilt the corrected bundle and restarted `site-ui-operations-prod`; the process is ready. Browser checks passed at 390px and 1365px for Events, Organizations, Rentals, and Teams. Each tab kept the viewport width with no horizontal overflow. Events exposed the measured sports and filter rows, More sports and More filters opened as dropdowns, and Price accepted `25` without losing focus. The resize case also passed: an open More sports menu remeasured after a desktop-to-mobile resize and closed without leaving a visible portal. The focused Discover suite passed 17 tests, changed-file lint passed, and the production build passed.
Revision note (2026-09-10, Issue 121 completion pass): Fixed server-sort pagination reset and stale-request guards, restored event distances, added explicit retry and accurate loading/error/empty/end states for all Discover lists and map results, added searchable sports, server-search field parity, date-range parity, scoped server totals, division-filter retry/loading states, 44px controls, map current-location and retry controls, grouped-result focus restoration, and active-tab map filtering. The final focused Discover run passed 8 suites and 92 tests. Changed-file lint passed with 0 errors and 9 complexity warnings. Workspace typechecking, full CI, full Jest, and production build remain blocked by unrelated existing Prisma/API errors, generated shared-icon drift, and the test setup MouseEvent failure. Browser smoke used the existing pre-edit runtime; it was not restarted.
