...
- [x] (2026-07-29 20:31Z) Built the deterministic mapping generator, isolated job worktree runner, evaluation harness, hash-verified offline page client, and disposable-database review scrape. The proof ran the same generated scraper twice, retained one candidate, left the mapping unvalidated, published nothing, contacted no public scrape target, and disposed its database and temporary worktrees.
...
- [ ] Complete `docs/affiliate-source-mapping-pretraining-validation-execplan.md`: freeze the 35-example real test suite, execute generated scrapers against disposable persistence, benchmark untouched bases, and reach an evidence-backed training-readiness decision.
...
- [x] (2026-07-29 20:31Z) Completed local implementation verification: all 62 affiliate-import suites passed 284 tests, the full coverage/route CI command passed, TypeScript and Prisma validation passed, the Python training utilities passed, Compose rendered with example deployment inputs, shell syntax passed, and `git diff --check` remained clean.
...
- Observation: none of the current local mappings qualifies automatically as a gold training example under the plan's provenance rules.
  Evidence: the 2026-07-29 dry run of `scripts/export-affiliate-mapping-training-data.ts` labeled 197 sources `LEGACY_PARTIAL`, 4 `STALE`, and 1 `BLOCKED`, with zero train-eligible rows. The backfill planner found 1 existing intake match, 1 blocked source, and 200 sources needing proposed intake reconstruction. A validation timestamp by itself is intentionally insufficient.

- Observation: the repository-wide TypeScript check became blocked by unrelated organization-matching work added to the shared branch while this milestone was in progress.
  Evidence: the three new affiliate-agent Jest suites passed 20 tests, while `npx tsc --noEmit --pretty false` reported errors only in `src/app/api/organizations/route.ts` and the untracked `src/server/organizationMatch.ts`, including unknown result types, unsupported `toSorted` for the current target, and ES2020 BigInt literals. The affiliate-agent files were not listed in the errors and the unrelated files were left untouched.

- Observation: a detached Git worktree has no `node_modules`, and Jest resolves its `ts-jest` preset relative to that worktree rather than the executable's original checkout.
  Evidence: the first fixture CLI run failed before tests with `Preset ts-jest not found relative to rootDir .../job-2bnfGz`. The validator now creates an ignored symlink from the detached worktree to the pinned current checkout's `node_modules`; the second run passed the contract and generated-source tests without installing or copying dependencies.
...
- Observation: `llama.cpp`'s native schema-constrained chat-completions request uses `response_format.type = "json_schema"` with the JSON Schema directly in `response_format.schema`.
  Evidence: the official server documentation describes the direct `schema` member and exposes prompt/output token throughput in the response `timings` object. The model client and VM verifier now use that native shape rather than the differently nested hosted-API convention.
...
- Observation: OpenAI's first published gpt-oss Transformers LoRA notebook was designed for one 80 GB H100, not one 48 GB L40S.
  Evidence: the official notebook uses `Mxfp4Config(dequantize=True)`, bfloat16, gradient checkpointing, all linear modules plus selected expert projections, and says smaller GPUs should reduce batch size and sequence length. The tracked L40S smoke therefore starts at batch size one and 2,048 tokens, runs one-example overfit before the bounded smoke, and treats fit on 48 GB as an experiment rather than an assumption.

- Observation: the first disposable scrape rejected its extracted event because the fixture mapping did not explicitly carry the expected evergreen date mode.
  Evidence: the run produced two successful scrape records but zero candidates because a missing `dateDisplayMode` defaults to scheduled-event validation. The fixture now maps `NO_FIXED_DATE`, its display text, sport, format, division, and tag through literal fields, proving that the generated mapping—not an out-of-band expected value—contains the data required for import.

- Observation: macOS reports temporary Git worktrees under the canonical `/private/var/...` path even when Node creates their parent under `/var/...`.
  Evidence: direct string comparison failed to select the generated worktrees for cleanup and left five stale registrations after the first attempts. Cleanup now canonicalizes the parent with `realpath`; the repeated passing run left no new worktree registration or database container.

- Observation: the deterministic control path now includes the real import service and Prisma persistence boundary.
  Evidence: `npm run affiliate:mapping:disposable-scrape` started digest-pinned PostgreSQL 17, applied migrations, generated and ran the scraper twice from clean detached worktrees, and verified one disabled source, one unvalidated mapping, one deduplicated candidate, zero published candidates, and two successful scrape runs. It made zero public scrape requests and zero live-database writes.
...
- Decision: generated setup files create a private, unlisted source organization without materializing or replacing its logo, and they never overwrite a later human-reviewed logo or public-listing state.
  Rationale: the draft can cite an official logo artifact, but normalization, storage, and rendered-fit review require asset bytes and human visual approval. A rerun of deterministic setup must not erase a subsequently approved logo or demote a public organization. The setup records logo evidence in source metadata, keeps automation disabled and mapping validation null, and leaves logo completion as an explicit review gate.
  Date/Author: 2026-07-29 / Codex
...
- Decision: make deterministic scrape integration use exact, hash-verified local page fixtures through an explicit environment variable.
  Rationale: the integration proof must exercise the production extractor and import service without contacting a public organization or accepting an arbitrary filesystem path. The fixture client accepts only exact manifest URLs, enforces a four-megabyte page limit and path containment, and verifies each page hash before return.
  Date/Author: 2026-07-29 / Codex

## Outcomes & Retrospective

The repository implementation is complete through the safe local control path: contracts, baseline and historical inventory, deterministic generation, bounded evidence access, isolated execution, real review-only persistence, structured independent review, private model-client integration, release construction, training scaffolding, VM deployment manifests, and bakeoff gates are committed. The deterministic proof now reaches the actual Prisma/import boundary and remains duplicate-safe and non-publishing.

The remaining work is deliberately external or evidence-dependent rather than missing repository plumbing. It requires an approved OVH order, exact downloaded open-weight artifacts and licenses, real-host measurements, a frozen private held-out suite, human-approved training examples, a working pinned Sol adapter, and authorized pilot runs. No OVH resource was purchased, no model weights were downloaded, no paid training was launched, and no live mapping claim or source mutation occurred during this implementation.
...
Implementation update 2026-07-29: added `src/server/affiliateImports/agentDataset.ts`, its focused tests, and the dry-run dataset/backfill CLIs. Dataset construction now uses deterministic domain-level splits and exact precedence for source-to-intake matching, refuses domain-only guesses on shared hosted platforms, and requires a provenance-linked intake run, policy artifacts, mapping approval, candidate outcome, setup script, and identifiable human approval before assigning `FAITHFUL`. The default CLIs perform no public requests or database writes; writing ignored reports requires an explicit flag.

Implementation update 2026-07-29: added constrained source templates, the byte-stable generator, provider-neutral model client, fixture worker, and evaluation scorecard. Executable drafts now render a source config, local-only setup script, focused generated test, and registry-note fragment; blocked, insufficient-evidence, and custom-extractor drafts cannot render executable files. Generated setup keeps `validatedAt = null` and `autoScrapeEnabled = false`, performs only explicit `--scrape` review runs, and does not overwrite human-reviewed logo or public organization state. The fixture evaluation proves safe refusal, artifact-bound citations, publish-critical field scoring, and hard-gate behavior before a real model is connected.

Implementation update 2026-07-29: added `AffiliateAgentToolbox` and the isolated draft runner. Evidence reads are now addressed by manifest SHA-256, verified again before return, byte-bounded, and restricted to text artifacts; repository reads and fixed-string searches stay inside an allowlist. The runner binds draft identity, policy, and evidence hashes to the claimed job, renders only controlled files, can create a detached Git worktree at a verified commit, and rejects diffs outside the source setup, generated config/test, and registry-fragment paths. Eleven focused tests cover tampering, path traversal, idempotent writes, unsafe identity/policy/evidence, and blocked-source refusal without file creation.
...
Implementation update 2026-07-29: added `deploy/ai` as a provider-specific but application-independent runtime package. The digest-pinned `llama.cpp` service has one slot, an 8,192-token context, quantized KV cache, API-key-file authentication, loopback-only host publishing, no Web UI or built-in agent tools, and an internal network with no egress. The separately credentialed controller defaults to disabled, claims at most one job, retains host-visible worktrees, and refuses live review scraping. Host checks enforce the VPS memory/core/disk/swap floor, pinned images, model hash, time synchronization, secret permissions, loopback binding, and absence of operational credentials in the model container. Model checks exercise schema-constrained refusal and record llama.cpp timing metrics. The Compose configuration, shell syntax, security-boundary tests, model-client tests, queue tests, context tests, and runner tests pass locally; no OVH resource or model artifact was created.
