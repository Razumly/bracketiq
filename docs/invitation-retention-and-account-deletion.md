...
Cleanup runs at the existing invitation listing boundary. Each call removes at most 250 eligible invitation attempts. It also removes their delivery records. Pending Event reconciliation can protect an invitation record until reconciliation is safe. That protection does not prevent cleanup of other eligible records. Ordinary history reads apply the retention cutoff even when physical cleanup has a backlog.
...
Invitation cleanup does not remove User Profiles, accepted Team Memberships, Team roster entries, or Event roster entries. It does not grant acceptance or Team access. Both clients refresh ordinary invitation history from the shared API. Mobile replaces that history in Room. Held review evidence is not sent to the mobile invitation cache.
