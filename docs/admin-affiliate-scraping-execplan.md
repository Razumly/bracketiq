...
Club-style sources are handled differently from event/program sources. When the source is a club directory or club website, the import candidate should publish a public BracketIQ organization page for that club instead of creating evergreen events. Tryouts or evaluations found on club sites should only become event candidates when the official source exposes a parseable future date; stale, missing-date, or not-yet-updated tryout content must not be converted into no-fixed-date evergreen events.
...
- [x] (2026-07-13 20:30Z) Added bounded daily change checks for enabled weekly/monthly sources and made the scheduled-run summary email send after every completed daily job, including days with no full scrape due.
- [x] (2026-07-06 18:30Z) Added `CLUB` import candidates so club sources can publish public organization pages, and documented that stale or missing-date tryouts must not become evergreen affiliate events.
- [x] (2026-07-09 02:40Z) Added `scripts/discover-club-affiliate-events.ts` as the second-pass club event discovery workflow. It scans published club orgs, checks robots.txt, follows likely tryout/camp/clinic/event links, rejects pages without source-provided future dates, writes only with `--write`, and records JSON audit reports under `output/affiliate-club-event-discovery/`.
- [x] (2026-07-09 02:40Z) Ran the club event discovery pass locally. OYSA dry run inspected 72 club rows and high-confidence writes created 5 discovered affiliate event candidates/backing unpublished events. CEVA dry run inspected 120 club rows; selected writes created 4 discovered affiliate event candidates/backing unpublished events. Metro VB Academy's Linktree URL is robots-blocked (`Disallow: /`) and was not scraped.
- [x] (2026-07-09 09:02Z) Added `scripts/discover-affiliate-club-logos.ts` as the directory-club logo repair workflow. It inspects official club websites only, checks robots.txt, ranks header/metadata/JSON-LD/icon assets, normalizes selected official assets into opaque 1024px PNGs, writes only with `--write`, supports `--min-score` for conservative bulk repair, and records JSON audit reports under `output/affiliate-club-logo-discovery/`. A first local high-confidence pass used `--min-score=100` and wrote 54 club logos plus the manually requested Gladstone Soccer Association repair; lower-confidence, directory-only, failed, or robots-blocked rows remain in the audit report for manual source lookup.
...
- Decision: Treat clubs as public organization imports, not evergreen event imports.
  Rationale: Club websites often contain tryout announcements, team pages, or stale season content. Publishing the club as an organization gives users a durable destination without inventing an always-open event. Tryout candidates still require source-provided future dates before they can become affiliate events.
  Date/Author: 2026-07-06 / Codex

- Decision: Club event discovery is a second pass over already-published club orgs.
  Rationale: Directory rows are not event rows. The first pass should create or repair the public club organization. A second pass may inspect the official club site for dated tryouts, camps, clinics, tournaments, or events, but it should write only normal unpublished `EVENT` candidates with source-provided future dates. Generic homepages, broad program pages, stale tryouts, missing-date pages, and robots-blocked targets stay in the audit report for manual review instead of becoming evergreen events.
...
- Decision: Use evergreen program events as the standard fallback for stale, vague, or non-date-specific source pages.
  Rationale: Some organization sites, such as Troutdale Indoor Sports, describe stable leagues, friendly games, tournaments, and rental processes but do not publish discrete current registration cards with parseable future start dates. Others leave old events on the page, mix outdated rows with active programs, or describe an event category poorly enough that importing it as a dated event would be misleading. For those sources, admins should not invent dates or fallback to scrape time. Instead, the mapping can emit manually approved summary candidates with `dateDisplayMode = NO_FIXED_DATE` or `ONGOING`, a clear `dateDisplayText` such as "No fixed start date" or "Call for availability", the official action URL, and source-derived event pricing. Public Discover includes these program listings by default, labels them as programs, and hides them when the user applies an explicit date range.
  Steps: First inspect the official page with ScrapingDog and, when useful, a rendered browser screenshot. If repeated cards include official action URLs plus future start dates or date ranges, build a normal selector mapping. If the page only describes ongoing programs, seasons, friendly games, rentals, tournaments, or categories without reliable current dated rows, or if the page contains stale/old event rows that cannot be separated safely from active opportunities, create a manual-summary mapping and record why the site was classified as evergreen. Preserve official prices by setting numeric `Events.price` to the selected or minimum source price, storing only a compact single price or price range in `Events.priceText`, and moving detailed source pricing terms, side fees, discounts, or late-fee wording into the event description/details. If neither pattern is trustworthy, leave the source in research or blocked status instead of creating misleading listings.
  Date/Author: 2026-06-30 / Codex

- Decision: Affiliate event price text must be display pricing, not source pricing prose.
  Rationale: Public cards and detail headers should compare affiliate events the same way normal BracketIQ events compare prices. Detailed pricing rules, side fees, late-fee dates, discount notes, player-card fees, uniform fees, and membership caveats belong in the event description/details, not in the card price area. Division prices remain the source of truth for multi-division affiliate events; the visible event price should be calculated from `Divisions.price` or the fallback numeric `Events.price`, not from `Events.priceText`.
  Date/Author: 2026-07-06 / Codex
...
In this plan, a "source" is a website or organization page configured for scraping. A source must be linked to a private BracketIQ organization before first use. A "mapping" is JSON stored in the database that tells the generic parser how to find repeated event, rental, team, or club items and how to extract fields from each item. A "scrape run" is one admin-triggered attempt to fetch and parse a source using its active mapping. A "candidate" is one discovered listing returned by a scrape run. A "published affiliate event" is a real `Events` row created after an admin approves an event candidate. A "published affiliate rental" is the public rental/facility/resource representation created after an admin approves a rental candidate. A "published affiliate club" is a public BracketIQ `Organizations` page created after an admin approves a club candidate. A "custom extractor" is optional TypeScript code used only when a source cannot be represented by a mapping.
...
For directory-imported clubs, logo capture is a separate review pass after the club org rows exist. Run `npm run affiliate:clubs:discover-logos` first as an audit, then use `--write --min-score=100` or a tighter org-specific command for high-confidence official assets. Do not assign generated initials or random placeholder logos to clubs. Rows whose website points back to a directory page, whose site is robots-blocked, or whose only candidates are low-score metadata/icons should stay missing until a manual official logo source is reviewed.

At minimum, persisted import candidate data needs source identity, source URL, official action URL, listing kind (`EVENT`, `RENTAL`, `TEAM`, or `CLUB`), title, organizer, sport, format, city, venue, address, date range, day/time text, skill level, age group, gender or division, team versus individual availability, price text, registration or booking status, registration deadline, source last-checked timestamp, source payload, admin review status, and the published target id such as `publishedEventId` for event candidates or `publishedOrganizationId` for club candidates. For scheduled event candidates, a parseable future `startsAt` from the source is required; do not fall back to the scrape date or current date. For approved evergreen program candidates, set `dateDisplayMode` to `NO_FIXED_DATE` or `ONGOING`, set `dateDisplayText` to the public availability label, leave `startsAt` null on the candidate, and store a numeric `Events.price` when the official source specifies a price. Do not apply the evergreen pattern to tryouts or evaluations; they require future source dates. The official action URL is copied to `Events.affiliateUrl` for published affiliate events.
...
The generic extractor returns normalized candidates:
    ...
    export type AffiliateCandidateInput = {
      listingKind: 'EVENT' | 'RENTAL' | 'TEAM' | 'CLUB';
      title: string;
...
Revision note: Added daily lightweight source monitoring for enabled weekly/monthly sources. These checks use conditional headers and a bounded response fingerprint, do not create candidates, and appear in the daily completion email as baselined, unchanged, changed, or failed.
